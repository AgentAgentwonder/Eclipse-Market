...
import { useWalletStore, WalletGroup, CreateGroupRequest, UpdateGroupRequest } from '../../store/walletStore'
...
export function GroupManagementModal({ isOpen, onClose }: GroupManagementModalProps) {
  ...
  const [formState, setFormState] = useState<{ name: string; description: string; walletIds: string[]; autoRebalance: boolean; maxSlippage?: number | null; defaultPriorityFee?: number | null; riskLevel?: string | null }>({
    name: '',
    description: '',
    walletIds: [],
    autoRebalance: false,
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-purple-500/20">
              <div>
                ...
                <p className="text-sm text-gray-400">Organize wallets into groups with shared trading preferences.</p>
              </div>
              <button
                onClick={() => {
                  resetForm()
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-gray-400">
                            <span>Max Slippage: {group.sharedSettings.maxSlippage ?? 'Default'}</span>
                            <span>Priority Fee: {group.sharedSettings.defaultPriorityFee ?? 'Default'}</span>
                            <span>Risk Level: {group.sharedSettings.riskLevel ?? 'Not set'}</span>
                            <span>Auto Rebalance: {group.sharedSettings.autoRebalance ? 'Enabled' : 'Disabled'}</span>
                          </div>
                        </div>
                        ...
                        return (
                          <label
                            ...
                            className={`flex items-center justify-between px-3 py-2 rounded-lg border transition-all ${
                              checked ? 'border-purple-500/60 bg-purple-500/10' : 'border-purple-500/20'
                            }`}
                          >
                            <div>
                              <div className="text-sm font-medium">{wallet.label}</div>
                              <div className="text-xs text-gray-400">{wallet.publicKey.slice(0, 4)}...{wallet.publicKey.slice(-4)}</div>
                            </div>
                            <input
                              type="checkbox"
                              checked={checked}
