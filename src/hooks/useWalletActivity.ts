...
import type { WalletActivity, ActivityFilter, MonitoredWallet, WalletStatistics } from '../types/insiders'
...
export function useMonitoredWallets() {
  ...
  const addWallet = useCallback(
    async (wallet_address: string, label?: string, is_whale: boolean = false, min_transaction_size?: number) => {
      try {
  ...
  const updateWallet = useCallback(
    async (
      id: string,
      updates: { label?: string; is_whale?: boolean; is_active?: boolean; min_transaction_size?: number }
    ) => {
      try {
