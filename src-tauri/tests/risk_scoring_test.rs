...
async fn test_risk_model_high_risk_token() {
    ...
    assert!(score > 60.0, "High risk token scored {}, expected > 60", score);
    ...
    assert!(
        factor_names.iter().any(|f| f.contains("gini") || f.contains("mint") || f.contains("freeze")),
...
async fn test_risk_model_low_risk_token() {
    ...
    assert!(score < 40.0, "Low risk token scored {}, expected < 40", score);
}
...
async fn test_risk_model_boundary_conditions() {
    ...
    assert!(score >= 0.0 && score <= 100.0, "Score {} out of valid range", score);
...
async fn test_model_serialization() {
    ...
    assert!((score1 - score2).abs() < 0.001, "Loaded model should produce same scores");
}
...
async fn test_risk_analyzer_scoring_and_storage() {
    ...
    let pool = SqlitePool::connect(&db_url).await.expect("Failed to connect to test DB");
    let analyzer = RiskAnalyzer::with_pool(pool).await.expect("Failed to create analyzer");
    ...
    let score = analyzer.score_token(token_address, features.clone()).await.expect("Scoring failed");
    ...
    let latest = analyzer.get_latest_risk_score(token_address).await.expect("Failed to get latest");
    ...
    let score2 = analyzer.score_token(token_address, features).await.expect("Second scoring failed");
    
    // Get history
    let history = analyzer.get_risk_history(token_address, 30).await.expect("Failed to get history");
...
async fn test_risk_analyzer_model_persistence() {
    ...
    analyzer.save_model(metrics).await.expect("Failed to save model");
    
    // Load it back
    analyzer.load_latest_model().await.expect("Failed to load model");
    ...
    let score = analyzer.score_token("test", features).await.expect("Scoring with loaded model failed");
    assert!(score.score >= 0.0 && score.score <= 100.0);
}
...
fn test_feature_extraction_correctness() {
    ...
    assert!(score >= 30.0 && score <= 90.0, "Score {} seems unreasonable", score);
    
    // Verify we have factors
    assert!(factors.len() > 0 && factors.len() <= 5, "Should have 1-5 factors");
...
fn test_risk_level_classification() {
    ...
    for (target_score, expected_level) in test_cases {
        ...
        let features = RiskFeatures {
            ...
            liquidity_usd: if target_score > 70.0 { 10000.0 } else { 1000000.0 },
            liquidity_to_mcap_ratio: 0.1,
