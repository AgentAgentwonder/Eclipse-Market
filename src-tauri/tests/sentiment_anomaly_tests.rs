...
fn test_sentiment_alert_thresholds() {
    ...
    assert!(!alerts.is_empty(), "Should generate alert for high positive sentiment");
}
...
fn test_anomaly_severity_classification() {
    ...
    let has_critical = anomalies.iter().any(|a| a.severity == "critical" || a.severity == "high");
    assert!(has_critical, "Should classify severe anomalies appropriately");
}
