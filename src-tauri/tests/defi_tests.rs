...
async fn marginfi_positions_format() {
    ...
    assert!(!positions.is_empty(), "marginfi positions should not be empty");
    for position in positions {
        assert!(position.value_usd >= 0.0, "position value must be non-negative");
    }
}

#[tokio::test]
async fn kamino_vaults_present() {
    let vaults = get_kamino_vaults().await.expect("should fetch kamino vaults");
