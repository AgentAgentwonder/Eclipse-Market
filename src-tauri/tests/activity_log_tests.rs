...
mod activity_log_tests {
    ...
    async fn test_log_activity() {
        ...
        logger
            .log_connect("test_wallet_address", details, true, Some("127.0.0.1".to_string()))
            .await
            .unwrap();

        let logs = logger
            .get_logs(ActivityLogFilter::default())
            .await
            .unwrap();
    ...
    async fn test_log_multiple_actions() {
        ...
        logger
            .log_send(wallet, json!({"amount": 1.5, "recipient": "recipient_address"}), true, None)
            .await
            .unwrap();
    ...
    async fn test_suspicious_activity() {
        ...
        let suspicious = logger
            .check_suspicious_activity(None)
            .await
            .unwrap();

        assert!(suspicious.iter().any(|s| s.activity_type == "rapid_connections"));
        assert!(suspicious.iter().any(|s| s.activity_type == "failed_signatures"));
