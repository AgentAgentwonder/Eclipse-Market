...
mod tests {
    ...
    use app_lib::trading::safety::policy::{SafetyPolicy, PolicyEngine};
    ...
    fn test_policy_enforcement() {
        ...
        let result = engine.check_trade_policy(
            "wallet1",
            2000.0,
            2.0,
            0.5,
        ...
        assert!(!result.allowed);
        assert!(result.violations.iter().any(|v| v.rule == "max_price_impact"));
        ...
        assert!(!result.allowed);
        assert!(result.violations.iter().any(|v| v.rule == "max_daily_trades"));
    }
    ...
    fn test_insurance_quote() {
        ...
        let quote = coordinator
            .request_quote("sol_shield", 50000.0, 2.0, 0.3);
        assert!(quote.is_ok());
        ...
        let quote = coordinator
            .request_quote("sol_shield", 100000.0, 5.0, 0.8);
        assert!(quote.is_ok());
        let quote = quote.unwrap();
    ...
    fn test_insurance_recommendation() {
        ...
        let recommendation = coordinator.recommend_provider(
            75000.0,
            3.5,
            0.4,
        );
    ...
    async fn test_safety_engine_high_risk_blocking() {
        ...
        assert!(check.policy_result.violations.iter().any(|v| v.rule == "high_risk_token"));
    }
