...
mod tests {
    ...
    async fn test_high_quality_token_prediction() {
        ...
        assert!(prediction.success_probability > 50.0, 
    ...
    async fn test_early_warnings_generation() {
        ...
        assert!(!warnings.is_empty(), "Risky token should generate early warnings");
        
        let warning_types: Vec<String> = warnings.iter()
            .map(|w| w.warning_type.clone())
            .collect();
        
        assert!(warning_types.contains(&"lowDeveloperReputation".to_string()) ||
    ...
    async fn test_predictor_persistence() {
        ...
        let prediction = predictor.predict("test_token_persist", features.clone()).await.unwrap();
        
        assert_eq!(prediction.token_address, "test_token_persist");
        
        let history = predictor.get_prediction_history("test_token_persist", 30).await.unwrap();
    ...
    async fn test_training_data_storage() {
        ...
        predictor.add_training_data("test_token_training", features.clone(), 0.85)
            .await
            .unwrap();
        
        predictor.add_training_data("test_token_training_2", 
            create_risky_features("test_token_training_2"), 0.15)
            .await
            .unwrap();
    }
    ...
    async fn test_model_retraining() {
        ...
        for i in 0..10 {
            ...
            predictor.add_training_data(&format!("train_token_{}", i), features, outcome)
                .await
                .unwrap();
        }
    ...
    async fn test_bias_detection_low_liquidity() {
        ...
        let has_liquidity_warning = warnings.iter()
            .any(|w| w.warning_type == "shallowLiquidity");
        
        assert!(has_liquidity_warning, "Should detect shallow liquidity as a bias risk");
    }
    ...
    async fn test_bias_detection_marketing_mismatch() {
        ...
        let has_marketing_warning = warnings.iter()
            .any(|w| w.warning_type == "marketingMismatch");
        
        assert!(has_marketing_warning, "Should detect marketing/engagement mismatch");
    }
    ...
    async fn test_confidence_score_calculation() {
        ...
        assert!(low_prediction.confidence < prediction.confidence,
