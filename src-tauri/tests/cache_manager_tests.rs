...
use app_lib::core::cache_manager::{
    CacheManager, CacheType, CacheTtlConfig, TimeProvider,
};
...
async fn test_cache_hit_and_miss() {
    ...
    // Set value
    manager.set(key.clone(), data.clone(), CacheType::TokenPrice).await.unwrap();
...
async fn test_ttl_expiration() {
    ...
    let result = manager.get(&key, CacheType::TokenPrice).await;
    assert!(result.is_none(), "Cache entry should have expired after TTL");
...
async fn test_lru_eviction() {
    ...
    manager.set("key_4".to_string(), json!({"value": 4}), CacheType::TokenPrice).await.unwrap();
...
async fn test_concurrent_access() {
    ...
    let manager = Arc::new(tokio::sync::RwLock::new(CacheManager::with_time_provider_and_path(
        100,
        1000,
        config_path,
        time_provider.clone(),
    ...
    for i in 0..10 {
        ...
        let handle = task::spawn(async move {
            ...
            mgr_read.set(key.clone(), data, CacheType::TokenPrice).await.unwrap();
            drop(mgr_read);
        });
        handles.push(handle);
    }
    ...
    let mgr_read = manager.read().await;
    mgr_read
        .set("shared_key".to_string(), json!({"shared": true}), CacheType::TokenPrice)
        .await
        .unwrap();
    drop(mgr_read);
