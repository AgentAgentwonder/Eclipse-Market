...
impl AlertManager {
    ...
    pub async fn process_whale_transaction(
        &self,
        ...
        if let Some(config) = config {
            if let Some(amount_usd) = activity.amount_usd {
                ...
                if amount_usd >= threshold {
                    ...
                    self.send_push_notification(config, &message, activity).await?;
                }
            }
        }
    ...
    async fn send_alert(&self, config: &AlertConfig, alert: &WhaleAlert) -> Result<(), String> {
        ...
        if config.push_enabled {
            let _ = self.app_handle.emit_all("whale_alert", alert);
        }
        ...
        if config.telegram_enabled {
            if let Some(telegram_config_id) = &config.telegram_config_id {
                let _ = self
                    .app_handle
                    .emit_all("send_telegram_alert", (telegram_config_id, &message));
            }
        }
    ...
    async fn send_push_notification(
        ...
        if config.push_enabled {
            let _ = self.app_handle.emit_all("smart_money_alert", activity);
        }
    ...
    pub async fn get_alert_configs(&self) -> Result<Vec<AlertConfig>, String> {
        ...
        for row in rows {
            let alert_type_str: String = row.try_get("alert_type").map_err(|e| format!("Failed to get alert_type: {e}"))?;
            ...
            configs.push(AlertConfig {
                ...
                alert_type,
                enabled: row.try_get::<i64, _>("enabled").map_err(|e| format!("Failed to get enabled: {e}"))? == 1,
                threshold: row.try_get("threshold").ok(),
                push_enabled: row.try_get::<i64, _>("push_enabled").map_err(|e| format!("Failed to get push_enabled: {e}"))? == 1,
                email_enabled: row.try_get::<i64, _>("email_enabled").map_err(|e| format!("Failed to get email_enabled: {e}"))? == 1,
                telegram_enabled: row.try_get::<i64, _>("telegram_enabled").map_err(|e| format!("Failed to get telegram_enabled: {e}"))? == 1,
