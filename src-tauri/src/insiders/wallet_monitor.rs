...
impl WalletMonitor {
    ...
    async fn attach_event_listener(&self) -> Result<(), String> {
        ...
        let event_handler = self.app_handle.listen_global("transaction_update", move |event| {
    ...
    pub async fn add_wallet(&self, request: AddMonitoredWalletRequest) -> Result<MonitoredWallet, String> {
    ...
    pub async fn update_wallet(&self, request: UpdateMonitoredWalletRequest) -> Result<MonitoredWallet, String> {
        let wallet = self
            .db
            .read()
            .await
    ...
    pub async fn get_wallet_statistics(&self, wallet_address: &str) -> Result<WalletStatistics, String> {
        self.db
            .read()
            .await
    ...
    pub async fn run_batch_processor(self: Arc<Self>) {
        ...
        loop {
            ...
            let _ = self.app_handle.emit_all("wallet_activity_batch", &batch);
        }
    }
                ...
                if let Some(info) = wallet_info {
                    if let Some(min_size) = info.min_transaction_size {
                        if amount_usd >= min_size {
                            let _ = self
                                .app_handle
                                .emit_all("wallet_large_transaction", &wallet_activity);
                        }
                    }
                }
            }
