...
impl SmartMoneyDetector {
    ...
    pub async fn classify_wallet(&self, wallet_address: &str) -> Result<SmartMoneyClassification, String> {
        ...
        let is_smart_money = metrics.total_trades >= 100 && metrics.win_rate >= 0.60 && score >= 60.0;
    ...
    async fn calculate_wallet_metrics(&self, wallet_address: &str) -> Result<SmartMoneyMetrics, String> {
        ...
        for activity in &trades {
            ...
            token_positions.entry(token_key).or_insert_with(Vec::new).push(TradeInfo {
    ...
    async fn get_wallet_trades(&self, wallet_address: &str) -> Result<Vec<WalletActivityRecord>, String> {
    ...
    pub async fn update_smart_money_wallet(
        ...
        let winning_trades = (classification.metrics.total_trades as f64 * classification.metrics.win_rate)
            .round()
        ...
        let total_pnl = classification.metrics.avg_profit_per_trade * classification.metrics.total_trades as f64;
    ...
    pub async fn is_smart_money(&self, wallet_address: &str) -> Result<bool, String> {
        let result = sqlx::query(
            "SELECT is_smart_money FROM smart_money_wallets WHERE wallet_address = ?1"
        )
        .bind(wallet_address)
        .fetch_optional(&self.pool)
        .await
        ...
        Ok(result.map(|row| row.try_get::<i64, _>("is_smart_money").unwrap_or(0) == 1).unwrap_or(false))
    }
    ...
    pub async fn get_consensus(&self, time_window_hours: i64) -> Result<Vec<SmartMoneyConsensus>, String> {
        ...
        for row in rows {
            ...
            consensus_list.push(SmartMoneyConsensus {
                ...
                consensus_strength,
                first_seen: row.try_get::<String, _>("first_seen")
                    .ok()
                    .and_then(|s| DateTime::parse_from_rfc3339(&s).ok())
                    .map(|dt| dt.with_timezone(&Utc))
                    .unwrap_or_else(|| Utc::now()),
                last_updated: row.try_get::<String, _>("last_updated")
                    .ok()
                    .and_then(|s| DateTime::parse_from_rfc3339(&s).ok())
                    .map(|dt| dt.with_timezone(&Utc))
                    .unwrap_or_else(|| Utc::now()),
    ...
    pub async fn get_sentiment_comparison(&self, token_mint: &str) -> Result<SentimentComparison, String> {
