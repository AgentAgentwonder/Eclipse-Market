...
#[tauri::command]
pub async fn classify_smart_money_wallet(wallet_address: String) -> Result<SmartMoneyClassification, String> {
...
#[tauri::command]
pub async fn get_smart_money_consensus(time_window_hours: i64) -> Result<Vec<SmartMoneyConsensus>, String> {
    let state = require_state()?;
    let detector = state.smart_money_detector.clone();
...
pub async fn scan_wallets_for_smart_money() -> Result<Vec<SmartMoneyClassification>, String> {
    ...
    for row in activities {
        let wallet_address: String = row.try_get("wallet_address").map_err(|e| format!("Failed to get wallet_address: {e}"))?;
        
        match detector.classify_wallet(&wallet_address).await {
            Ok(classification) => {
                if classification.is_smart_money {
                    let _ = detector.update_smart_money_wallet(&classification, None).await;
