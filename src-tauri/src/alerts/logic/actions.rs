    ...
    pub fn validate(&self) -> Result<(), String> {
        match self.action_type {
            ActionType::SendEmail => {
                if self.parameters.email_to.is_none() || self.parameters.email_to.as_ref().unwrap().is_empty() {
            ...
            ActionType::ExecuteTrade => {
                ...
                if trade_config.amount.is_none() && trade_config.amount_percent.is_none() {
                    return Err("Trade config requires either 'amount' or 'amount_percent'".to_string());
                }
            }
