...
impl DryRunSimulator {
    pub fn simulate_rule(
        ...
        if evaluation.triggered && rule.actions.iter().any(|a| a.action_type == ActionType::ExecuteTrade) {
    ...
    fn estimate_action_impact(
        ...
        match action.action_type {
            ...
            ActionType::SendDiscord => {
                Some("Would send notification".to_string())
            }
            ...
            ActionType::UpdateAlert => {
                Some(format!("Would update alert: {}", rule.name))
            }
            ActionType::LogEvent => {
                Some("Would log event to system".to_string())
            }
        }
    }
    ...
    pub fn generate_test_scenarios(symbol: &str, current_price: f64) -> Vec<(MarketData, Option<WhaleActivity>)> {
        vec![
            (
                MarketData {
        ...
        for action in &rule.actions {
            ...
            let result = if dry_run {
                ActionExecutionResult {
                    ...
                    success: true,
                    message: format!("DRY RUN: Would execute {} action", action.action_type.as_str()),
                    error: None,
