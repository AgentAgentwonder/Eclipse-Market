...
pub async fn get_top_movers(
    ...
    // Update cache
    cache_guard.top_movers.insert(cache_key, (movers.clone(), std::time::SystemTime::now()));

    Ok(movers)
}

#[tauri::command]
pub async fn get_new_ipos(
    cache: State<'_, SharedStockCache>,
) -> Result<Vec<NewIPO>, String> {
    // Check cache first
    let mut cache_guard = cache.write().await;
...
pub async fn get_stock_news(
    ...
    // Update cache
    cache_guard.news_cache.insert(symbol, (news.clone(), std::time::SystemTime::now()));
...
#[tauri::command]
pub async fn get_insider_activity(
    symbol: String,
