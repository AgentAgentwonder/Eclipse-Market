...
impl BridgeManager {
    ...
    pub fn list_transactions_by_status(&self, status: &BridgeTransactionStatus) -> Vec<BridgeTransaction> {
        self.transactions
            .values()
    ...
    pub fn update_transaction_status(&mut self, id: &str, status: BridgeTransactionStatus) -> Result<(), String> {
        if let Some(tx) = self.transactions.get_mut(id) {
            tx.status = status;
            tx.updated_at = chrono::Utc::now().to_rfc3339();
            if matches!(tx.status, BridgeTransactionStatus::Completed | BridgeTransactionStatus::Failed) {
