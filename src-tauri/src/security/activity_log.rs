...
impl ActivityLogger {
    ...
    pub async fn log_connect<T: SerializeValue + Send + Sync>(
        ...
        self.log_activity(wallet_address, ActivityAction::Connect, details, result, ip_address)
            .await
    }
    ...
    pub async fn log_disconnect<T: SerializeValue + Send + Sync>(
        ...
        self.log_activity(wallet_address, ActivityAction::Disconnect, details, result, ip_address)
            .await
    }
    ...
    pub async fn log_sign<T: SerializeValue + Send + Sync>(
        ...
        self.log_activity(wallet_address, ActivityAction::Sign, details, result, ip_address)
            .await
    }
    ...
    pub async fn log_send<T: SerializeValue + Send + Sync>(
        ...
        self.log_activity(wallet_address, ActivityAction::Send, details, result, ip_address)
            .await
    }
    ...
    pub async fn log_swap<T: SerializeValue + Send + Sync>(
        ...
        self.log_activity(wallet_address, ActivityAction::Swap, details, result, ip_address)
            .await
    }
    ...
    pub async fn log_approve<T: SerializeValue + Send + Sync>(
        ...
        self.log_activity(wallet_address, ActivityAction::Approve, details, result, ip_address)
            .await
    }
    ...
    pub async fn log_reject<T: SerializeValue + Send + Sync>(
        ...
        self.log_activity(wallet_address, ActivityAction::Reject, details, result, ip_address)
            .await
    }
    ...
    pub async fn export_to_csv(
        ...
        let mut csv = String::from("id,wallet_address,action,details,ip_address,timestamp,result\n");
    ...
    pub async fn get_stats(
        ...
        let suspicious = self.check_suspicious_activity(wallet_address.clone()).await?;
    ...
    pub async fn check_suspicious_activity(
        ...
        for row in rapid_rows {
            ...
            suspicious.push(SuspiciousActivity {
                ...
                description: format!(
                    "Detected {} connect/disconnect events in 1 minute",
                    count
                ),
        ...
        for row in failed_rows {
            ...
            suspicious.push(SuspiciousActivity {
                ...
                description: format!(
                    "Detected {} failed signature attempts in 5 minutes",
                    count
                ),
                timestamp: now.to_rfc3339(),
                wallet_address: wallet,
        ...
        for row in volume_rows {
            ...
            suspicious.push(SuspiciousActivity {
                ...
                description: format!(
                    "Detected {} send/swap operations in 1 minute",
                    count
                ),
                timestamp: now.to_rfc3339(),
                wallet_address: wallet,
    ...
    pub fn current_retention_days(&self) -> Result<i64, ActivityLogError> {
        self.retention_days
            .read()
            .map(|guard| *guard)
            .map_err(|_| ActivityLogError::Internal("Failed to read retention configuration".to_string()))
    }
    ...
    pub fn set_retention_days(&self, days: i64) -> Result<i64, ActivityLogError> {
        let validated = validate_retention(days)?;
        {
            let mut guard = self
                .retention_days
                .write()
                .map_err(|_| ActivityLogError::Internal("Failed to update retention configuration".to_string()))?;
            *guard = validated;
        }
...
fn activity_log_path(app: &AppHandle) -> Result<PathBuf, ActivityLogError> {
    let mut path = app
        .path_resolver()
        .app_data_dir()
        .ok_or_else(|| ActivityLogError::Internal("Unable to resolve app data directory".to_string()))?;
    if !path.exists() {
...
fn persist_retention_days(path: &Path, days: i64) -> Result<(), ActivityLogError> {
    ...
    let config = ActivityLogConfig { retention_days: days };
...
fn validate_retention(days: i64) -> Result<i64, ActivityLogError> {
    if days <= 0 {
        return Err(ActivityLogError::InvalidRetention("Retention must be greater than zero".to_string()));
    }
...
pub async fn export_activity_logs(
    ...
    logger.export_to_csv(filter).await.map_err(|e| e.to_string())
}
