        ...
        if let Some(code) = program_code {
            ...
            if code.contains("selfdestruct") || code.contains("suicide") {
                findings.push(Finding {
                    ...
                    description: "Contract contains selfdestruct function which can destroy the contract".to_string(),
                    recommendation: Some("Avoid interacting with contracts that can be destroyed".to_string()),
            ...
            if code.contains("delegatecall") {
                findings.push(Finding {
                    ...
                    description: "Contract uses delegatecall which can be risky if not properly implemented".to_string(),
                    recommendation: Some("Review the delegatecall implementation carefully".to_string()),
                    source: "Heuristic".to_string(),
                });
            }
            ...
            if code.contains(".call(") && !code.contains("require(") {
                findings.push(Finding {
                    ...
                    description: "External calls without proper error handling detected".to_string(),
    ...
    pub fn analyze_metadata(metadata: &AuditMetadata) -> Vec<Finding> {
        ...
        if metadata.is_mintable {
            findings.push(Finding {
                ...
                recommendation: Some("Be aware that supply can increase, potentially diluting value".to_string()),
                source: "Heuristic".to_string(),
            });
        }
        ...
        if metadata.has_blacklist {
            findings.push(Finding {
                ...
                description: "Token can blacklist addresses, preventing them from transferring".to_string(),
...
impl CertikClient {
    pub async fn fetch_audit(contract_address: &str) -> Result<AuditSource, String> {
        ...
        Ok(AuditSource {
            ...
            status: if score > 0 { AuditStatus::Verified } else { AuditStatus::Unavailable },
            score: if score > 0 { Some(score) } else { None },
            last_updated: if score > 0 { Some(Utc::now()) } else { None },
            report_url: if score > 0 {
                Some(format!("https://skynet.certik.com/projects/{}", contract_address))
            } else {
                None
            },
        })
...
impl TrailOfBitsClient {
    pub async fn fetch_audit(contract_address: &str) -> Result<AuditSource, String> {
        ...
        Ok(AuditSource {
            ...
            last_updated: if score > 0 { Some(Utc::now()) } else { None },
            report_url: if score > 0 {
                Some(format!("https://www.trailofbits.com/audits/{}", contract_address))
            } else {
                None
            },
        })
...
pub async fn perform_audit(contract_address: &str, metadata: AuditMetadata) -> Result<AuditResult, String> {
    let mut findings = Vec::new();
    ...
    let external_scores: Vec<u8> = audit_sources
        .iter()
...
async fn fetch_token_metadata(contract_address: &str) -> Result<AuditMetadata, String> {
    ...
    let hash = contract_address.bytes().fold(0u64, |acc, b| acc.wrapping_add(b as u64));
