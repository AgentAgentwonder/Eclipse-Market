...
impl ReputationEngine {
    pub async fn new(app_handle: &AppHandle) -> Result<Self, ReputationError> {
        let app_dir = app_handle
            .path_resolver()
            .app_data_dir()
            .ok_or_else(|| ReputationError::Io(std::io::Error::new(
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_wallet_trust ON wallet_reputation(trust_score)")
            .execute(&pool)
            .await?;
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_history_address ON reputation_history(address)")
            .execute(&pool)
            .await?;
    ...
    pub async fn get_wallet_reputation(&self, address: &str) -> Result<WalletReputation, ReputationError> {
        let row = sqlx::query(
            r#"
    ...
    async fn initialize_wallet_reputation(&self, address: &str) -> Result<(), ReputationError> {
        ...
        self.record_history(address, 50.0, "initialized", None).await?;
        Ok(())
    }
    ...
    pub async fn update_wallet_behavior(
        ...
        self.record_history(address, trust_score, "behavior_update", None).await?;
        Ok(())
    }
    ...
    pub async fn get_token_reputation(&self, address: &str) -> Result<TokenReputation, ReputationError> {
        let row = sqlx::query(
            r#"
        ...
        match row {
            ...
            None => Err(ReputationError::NotFound(format!("Token not found: {}", address))),
        }
    }
    ...
    pub async fn initialize_token_reputation(
        ...
        self.record_history(address, 50.0, "token_created", None).await?;
        Ok(())
    }
    ...
    pub async fn update_token_metrics(
        ...
        self.record_history(address, trust_score, "metrics_update", None).await?;
        Ok(())
    }
    ...
    pub async fn add_vouch(
        ...
        self.record_history(target_address, 0.0, "vouch_received", Some(&format!("from: {}", voucher_address))).await?;
        Ok(())
    }
    ...
    pub async fn get_vouches(&self, target_address: &str) -> Result<Vec<VouchRecord>, ReputationError> {
        let rows = sqlx::query(
            r#"
    ...
    pub async fn add_to_blacklist(
        ...
        self.record_history(address, 0.0, "blacklisted", Some(reason)).await?;
        Ok(())
    }

    pub async fn remove_from_blacklist(&self, address: &str, entry_type: &str) -> Result<(), ReputationError> {
        sqlx::query(
            r#"
            UPDATE blacklist
            SET is_active = 0
        ...
        self.record_history(address, 0.0, "removed_from_blacklist", None).await?;
        Ok(())
    }

    pub async fn get_blacklist(&self, entry_type: Option<String>) -> Result<Vec<BlacklistEntry>, ReputationError> {
    ...
    pub async fn submit_report(&self, report: ReputationReport) -> Result<(), ReputationError> {
        ...
        let report_count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM reputation_reports WHERE target_address = ?"
        )
        .bind(&report.target_address)
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn get_stats(&self) -> Result<ReputationStats, ReputationError> {
        ...
        let total_vouches: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM vouches WHERE is_active = 1")
            .fetch_one(&self.pool)
            .await?;

        let total_blacklisted: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM blacklist WHERE is_active = 1")
            .fetch_one(&self.pool)
            .await?;
        ...
        let avg_score: Option<f64> = sqlx::query_scalar("SELECT AVG(trust_score) FROM wallet_reputation")
            .fetch_one(&self.pool)
            .await
            .ok();
...
pub async fn get_wallet_reputation(
    ...
    engine.get_wallet_reputation(&address).await.map_err(|e| e.to_string())
}
...
pub async fn get_token_reputation(
    ...
    let engine = engine.read().await;
    engine.get_token_reputation(&address).await.map_err(|e| e.to_string())
}
...
pub async fn get_vouches(
    ...
    let engine = engine.read().await;
    engine.get_vouches(&target_address).await.map_err(|e| e.to_string())
}
...
pub async fn get_blacklist(
    ...
    let engine = engine.read().await;
    engine.get_blacklist(entry_type).await.map_err(|e| e.to_string())
}
...
pub async fn submit_reputation_report(
    ...
    let engine = engine.read().await;
    engine.submit_report(report).await.map_err(|e| e.to_string())
}
...
mod tests {
    ...
    fn test_reputation_level_from_score() {
        assert_eq!(ReputationLevel::from_score(95.0), ReputationLevel::Excellent);
        ...
        assert_eq!(ReputationLevel::from_score(10.0), ReputationLevel::Malicious);
    }
}
