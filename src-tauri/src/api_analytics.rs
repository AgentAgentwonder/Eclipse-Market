...
impl ApiUsageTracker {
    ...
    fn initialize_fair_use_limits(&self) -> Result<(), String> {
        ...
        for (service, daily, monthly) in services {
            limits.insert(service.to_string(), FairUseLimit {
    ...
    pub fn get_analytics(&self, days: i64) -> Result<ApiUsageAnalytics, String> {
        ...
        for record in &recent_records {
            let entry = services.entry(record.service.clone()).or_insert(UsageStats {
        ...
        for (service, stats) in services.iter_mut() {
            let service_records: Vec<_> = recent_records.iter()
            ...
            for record in &service_records {
                let entry = endpoint_map.entry(record.endpoint.clone()).or_insert((0, 0, 0));
                entry.0 += 1; // count
                entry.1 += record.latency_ms; // total latency
                entry.2 += if record.status_code >= 200 && record.status_code < 300 { 1 } else { 0 }; // success count
            }
    ...
    fn generate_alerts(&self, services: &HashMap<String, UsageStats>) -> Result<Vec<UsageAlert>, String> {
        ...
        for (service, limit) in limits.iter() {
            // Check daily limit
            let daily_usage_percent = (limit.current_daily_usage as f64 / limit.daily_limit as f64) * 100.0;
