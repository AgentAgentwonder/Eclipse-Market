...
impl DbRepairModule {
                ...
                if let Ok(metadata) = std::fs::metadata(&db_path) {
                    ...
                    if self.check_db_integrity(&db_path).is_err() {
                        ...
                        issues.push(DiagnosticIssue {
                            ...
                            description: format!("Database file {} appears to be corrupted", db_file),
                            detected_at: Utc::now(),
                            ...
                            metadata: {
                                ...
                                m.insert("path".to_string(), serde_json::Value::String(db_path.display().to_string()));
                                m
                            },
                        });
                    }
    ...
    pub async fn auto_repair(&self, issue: &DiagnosticIssue, app_data_dir: &Path) -> Result<AutoRepairResult, String> {
        if issue.title.starts_with("Corrupted database:") {
            if let Some(path_value) = issue.metadata.get("path") {
                if let Some(path_str) = path_value.as_str() {
                    ...
                    // Create backup
                    let backup_path = format!("{}.backup.{}", db_path.display(), Utc::now().timestamp());
                    ...
                    return Ok(AutoRepairResult {
                        ...
                        actions: vec![
                            ...
                            RepairAction {
                                ...
                                description: "Database will be recreated on next app restart".to_string(),
