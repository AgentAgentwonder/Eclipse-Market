...
impl DiagnosticsEngine {
    ...
    pub async fn auto_repair(&mut self, issue: &DiagnosticIssue) -> Result<AutoRepairResult, String> {
        ...
        let result = match issue.category {
            ...
            IssueCategory::Dependencies => {
                if let Some(packages) = self.dependency_manager.auto_repair(issue, &self.project_root) {
                    self.dependency_manager
                        .install_dependencies(&self.project_root, if packages.is_empty() { None } else { Some(packages) })
                        .await
                } else {
            ...
            IssueCategory::CodeIntegrity | IssueCategory::Configuration => self.code_repair.auto_repair(issue),
            IssueCategory::Network => self.network_diagnostics.auto_switch_endpoint(issue).await,
            IssueCategory::Performance => self.performance_repair.auto_repair(issue, &self.app_data_dir),
            _ => Err(format!("No auto-repair available for category: {:?}", issue.category)),
        };
    ...
    fn record_repair(&mut self, issue: Option<&DiagnosticIssue>, result: &AutoRepairResult, started_at: DateTime<Utc>) {
        let issue_id = issue
            .map(|i| i.id.clone())
    ...
    pub async fn install_dependencies(&mut self, packages: Option<Vec<String>>) -> Result<AutoRepairResult, String> {
        let started_at = Utc::now();
        let result = self
            .dependency_manager
    ...
    pub fn generate_repair_plan(&self, issue_id: &str) -> Result<RepairPlan, String> {
        ...
        let requires_backup = matches!(issue.category, IssueCategory::Database | IssueCategory::FileSystem);
        ...
        Ok(RepairPlan {
            ...
            steps,
            requires_backup,
            estimated_time: if requires_backup { "5-10 minutes".to_string() } else { "2-5 minutes".to_string() },
