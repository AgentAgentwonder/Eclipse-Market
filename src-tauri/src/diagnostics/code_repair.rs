...
impl CodeRepairModule {
    ...
    fn validate_theme_files(&self, theme_dir: &Path, issues: &mut Vec<DiagnosticIssue>) -> HealthLevel {
        if !theme_dir.exists() {
        ...
        if let Ok(entries) = fs::read_dir(theme_dir) {
            for entry in entries.flatten() {
                if let Some(ext) = entry.path().extension() {
                    if ext == "json" {
                        if let Err(issue) = self.validate_json_file(&entry.path(), IssueCategory::CodeIntegrity) {
    ...
    fn validate_config_files(
        ...
        if let Ok(entries) = fs::read_dir(config_dir) {
            for entry in entries.flatten() {
                if let Some(ext) = entry.path().extension() {
                    if ext == "json" {
                        if let Err(issue) = self.validate_json_file(&entry.path(), IssueCategory::Configuration) {
                            health = HealthLevel::Degraded;
                            issues.push(issue);
                        }
                    } else if ext == "toml" {
    ...
    fn validate_json_file(&self, path: &Path, category: IssueCategory) -> Result<(), DiagnosticIssue> {
        match fs::read_to_string(path) {
            Ok(content) => match serde_json::from_str::<Value>(&content) {
                ...
                Err(err) => Err(DiagnosticIssue {
                    ...
                    title: format!("Invalid JSON: {}", path.file_name().unwrap_or_default().to_string_lossy()),
                    ...
                    metadata: {
                        ...
                        m.insert("path".into(), serde_json::Value::String(path.display().to_string()));
                        m
                    },
                }),
            },
            Err(err) => Err(DiagnosticIssue {
                ...
                category,
                severity: IssueSeverity::Warning,
                title: format!("Unreadable JSON: {}", path.file_name().unwrap_or_default().to_string_lossy()),
    ...
    fn validate_toml_file(&self, path: &Path) -> Result<(), DiagnosticIssue> {
        match fs::read_to_string(path) {
            ...
            Err(err) => Err(DiagnosticIssue {
                ...
                severity: IssueSeverity::Warning,
                title: format!("Unreadable TOML: {}", path.file_name().unwrap_or_default().to_string_lossy()),
    ...
    fn validate_component_file(&self, path: &Path) -> Result<(), DiagnosticIssue> {
        let metadata = fs::metadata(path).map_err(|err| DiagnosticIssue {
            ...
            severity: IssueSeverity::Warning,
            title: format!("Unreadable component: {}", path.file_name().unwrap_or_default().to_string_lossy()),
            description: format!("Failed to access component file {}: {}", path.display(), err),
            detected_at: Utc::now(),
            recommended_action: "Check file permissions".to_string(),
            repair_level: RepairLevel::Confirmation,
            auto_repair_available: false,
        ...
        if metadata.len() == 0 {
            return Err(DiagnosticIssue {
                ...
                category: IssueCategory::CodeIntegrity,
                severity: IssueSeverity::Warning,
                title: format!("Empty component: {}", path.file_name().unwrap_or_default().to_string_lossy()),
                        ...
                        if let Ok(value) = serde_json::from_str::<Value>(&content) {
                            ...
                            return Ok(AutoRepairResult {
                                ...
                                actions: vec![RepairAction {
                                    ...
                                    description: format!("Auto-formatted JSON at {}", path.display()),
                        ...
                        if let Ok(value) = toml::from_str::<toml::Value>(&content) {
                            ...
                            return Ok(AutoRepairResult {
                                ...
                                actions: vec![RepairAction {
                                    ...
                                    description: format!("Auto-formatted TOML at {}", path.display()),
                                    estimated_duration: Some("< 1 second".to_string()),
                                    level: RepairLevel::Automatic,
                                    requires_confirmation: false,
                                }],
