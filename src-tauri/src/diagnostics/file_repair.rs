...
impl FileRepairModule {
    ...
    pub fn diagnose(&self, app_data_dir: &Path) -> ModuleDiagnostics {
        ...
        let directories_ok = if app_data_exists {
            ...
            issues.push(DiagnosticIssue {
                ...
                description: format!("The app data directory at {:?} does not exist", app_data_dir),
                detected_at: Utc::now(),
                recommended_action: "Create app data directory and restore default files".to_string(),
    ...
    fn check_required_directories(&self, base: &Path, issues: &mut Vec<DiagnosticIssue>, notes: &mut Vec<String>) -> bool {
        ...
        for dir in &required_dirs {
            ...
            if !path.exists() {
                ...
                issues.push(DiagnosticIssue {
                    ...
                    description: format!("Required directory '{}' does not exist at {:?}", dir, path),
                    detected_at: Utc::now(),
                    ...
                    metadata: {
                        ...
                        m.insert("path".to_string(), serde_json::Value::String(path.display().to_string()));
                        m
                    },
                });
            }
    ...
    fn check_file_permissions(&self, _base: &Path, _issues: &mut Vec<DiagnosticIssue>, notes: &mut Vec<String>) -> bool {
    ...
    fn check_config_files(&self, base: &Path, issues: &mut Vec<DiagnosticIssue>, notes: &mut Vec<String>) -> bool {
                ...
                match fs::read_to_string(&path) {
                    Ok(content) => {
                        if let Err(e) = serde_json::from_str::<serde_json::Value>(&content) {
                            ...
                            issues.push(DiagnosticIssue {
                                ...
                                description: format!("Config file '{}' is not valid JSON: {}", config_path, e),
                                detected_at: Utc::now(),
                                recommended_action: "Repair or reset configuration file".to_string(),
                    ...
                    Err(e) => {
                        ...
                        issues.push(DiagnosticIssue {
                            ...
                            description: format!("Cannot read config file '{}': {}", config_path, e),
                            detected_at: Utc::now(),
    ...
    pub fn auto_repair(&self, issue: &DiagnosticIssue, app_data_dir: &Path) -> Result<AutoRepairResult, String> {
        match issue.title.as_str() {
            ...
            title if title.starts_with("Corrupted config:") || title.starts_with("Unreadable config:") => {
                if let Some(path_value) = issue.metadata.get("path") {
                    if let Some(path_str) = path_value.as_str() {
                        let path = PathBuf::from(path_str);
                        // Create backup first
                        ...
                        return Ok(AutoRepairResult {
                            ...
                            actions: vec![
                                ...
                                RepairAction {
                                    ...
                                    description: format!("Reset config file to defaults: {:?}", path),
                                    estimated_duration: Some("< 1 second".to_string()),
                                    level: RepairLevel::Automatic,
                                    requires_confirmation: false,
                                },
        ...
        Err(format!("No auto-repair available for issue: {}", issue.title))
    }
}
