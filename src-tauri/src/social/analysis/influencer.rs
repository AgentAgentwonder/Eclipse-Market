...
impl InfluencerEngine {
    ...
    pub async fn compute_influencer_scores(
        ...
        for (influencer, stats) in influencer_data {
            ...
            let follower_score = self.normalize_follower_score(stats.engagement_total as f32, max_engagement);
            ...
            let accuracy_score = (1.0 - ((avg_sentiment - snapshot_avg).abs() / 2.0)).clamp(0.0, 1.0);
