...
impl GaugeEngine {
    ...
    pub async fn fetch_gauges(
        ...
        let mut readings = Vec::new();
        for row in rows {
            let drivers_json: String = row.try_get("drivers")?;
            let drivers: HashMap<String, f32> = serde_json::from_str(&drivers_json).unwrap_or_default();

            readings.push(GaugeReading {
                token: row.try_get("token")?,
                fomo_score: row.try_get("fomo_score")?,
                fomo_level: row.try_get("fomo_level")?,
                fud_score: row.try_get("fud_score")?,
                fud_level: row.try_get("fud_level")?,
                drivers,
