...
impl SocialDataService {
    pub async fn new(app: &AppHandle) -> Result<Self, SocialError> {
        ...
        let mut data_dir = app
            .path_resolver()
            .app_data_dir()
            .ok_or_else(|| SocialError::Internal("Failed to resolve app data directory".to_string()))?;
    ...
    pub async fn fetch_reddit(
        ...
        self.cache
            .store_posts(&result.posts, token)
            .await?;
    ...
    pub async fn fetch_twitter(
        ...
        self.cache
            .store_posts(&result.posts, token_address)
            .await?;
    ...
    fn resolve_bearer_token(
        ...
        let store = keystore.ok_or_else(|| {
            SocialError::Internal("Keystore state unavailable for Twitter authentication".to_string())
        })?;

        TwitterClient::get_bearer_token_from_keystore(store).map_err(SocialError::from)
    }

    pub fn set_twitter_bearer_token(&self, keystore: &Keystore, token: &str) -> Result<(), SocialError> {
    ...
    pub async fn get_cached_posts(
        ...
        self.cache.get_cached_posts(source, token, limit).await.map_err(Into::into)
    }
    ...
    pub async fn create_trend_snapshot(&self, token: &str, source: &str) -> Result<(), SocialError> {
        self.cache
