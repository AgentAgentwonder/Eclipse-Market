...
impl SocialCache {
    ...
    async fn update_mention_aggregates(
        ...
        let source = posts.first().map(|p| p.source.as_str()).unwrap_or("unknown");
        let now = Utc::now().timestamp();
    ...
    pub async fn get_mention_aggregates(
        ...
        let query = if let Some(tok) = token {
            ...
            sqlx::query(
                "SELECT * FROM mention_aggregates ORDER BY last_updated DESC LIMIT 100"
            )
        };
    ...
    pub async fn create_trend_snapshot(
        ...
        let posts = self.get_cached_posts(Some(source), Some(token), Some(1000)).await?;
