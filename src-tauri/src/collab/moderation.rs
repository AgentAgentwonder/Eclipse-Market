...
use crate::collab::types::{ModerationAction, ModerationActionType, ParticipantPermissions, ParticipantRole};
...
impl ModerationManager {
    ...
    pub fn record_action(&self, action: ModerationAction) {
        if action.action_type == ModerationActionType::Ban {
            self.banned_users.write().insert(action.target_user_id.clone());
        }

        let mut map = self.actions.write();
        map.entry(action.room_id)
            .or_default()
            .push(action);
    }
