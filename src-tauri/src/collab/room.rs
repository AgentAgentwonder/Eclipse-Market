...
use crate::collab::types::{
    ChatMessage, CreateRoomRequest, JoinRoomRequest, Participant, ParticipantRole,
...
impl RoomManager {
    ...
    pub fn join_room(
        ...
        if let Some(password_hash) = &room.password_hash {
            let password = request.password.ok_or_else(|| anyhow!("Password required"))?;
    ...
    pub fn update_participant(&self, participant: Participant) -> Result<()> {
        ...
        if let Some(room_participants) = participants.get_mut(&participant.room_id) {
            if let Some(p) = room_participants.iter_mut().find(|p| p.id == participant.id) {
                *p = participant;
                Ok(())
            } else {
    ...
    pub fn send_message(&self, request: SendMessageRequest, user_id: String, username: String) -> Result<ChatMessage> {
        let room = self.get_room(&request.room_id)?;
