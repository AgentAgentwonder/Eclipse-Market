...
impl RewardEngine {
    pub async fn new(app_handle: &AppHandle) -> Result<Self, RewardError> {
        let app_dir = app_handle
            .path_resolver()
            .app_data_dir()
            .ok_or_else(|| RewardError::Io(std::io::Error::new(
    ...
    async fn init_schema(pool: &Pool<Sqlite>) -> Result<(), RewardError> {
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_earned_badges_wallet ON earned_badges(wallet_address)")
            .execute(pool)
            .await?;
        
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_certificates_wallet ON certificates(wallet_address)")
            .execute(pool)
            .await?;
    ...
    // Award badge to user
    pub async fn award_badge(&self, wallet_address: &str, badge_id: &str, source: &str) -> Result<EarnedBadge, RewardError> {
        ...
        if existing.unwrap_or(0) > 0 {
            return Err(RewardError::AlreadyClaimed("Badge already earned".to_string()));
        }
    ...
    pub async fn get_user_badges(&self, wallet_address: &str) -> Result<Vec<EarnedBadge>, RewardError> {
        let rows = sqlx::query(
    ...
    pub async fn issue_certificate(&self, certificate: Certificate) -> Result<Certificate, RewardError> {
        sqlx::query(
            r#"
    ...
    pub async fn get_user_certificates(&self, wallet_address: &str) -> Result<Vec<Certificate>, RewardError> {
        let rows = sqlx::query(
    ...
    pub async fn verify_certificate(&self, verification_code: &str) -> Result<Certificate, RewardError> {
        let row = sqlx::query(
    ...
    pub async fn get_user_rewards(&self, wallet_address: &str, unclaimed_only: bool) -> Result<Vec<Reward>, RewardError> {
    ...
    pub async fn claim_reward(&self, reward_id: &str) -> Result<(), RewardError> {
        ...
        if result.rows_affected() == 0 {
            return Err(RewardError::AlreadyClaimed("Reward already claimed or not found".to_string()));
        }
    ...
    pub async fn calculate_quiz_reward(&self, score: i64, total_points: i64, time_bonus: bool) -> i64 {
    ...
    pub async fn calculate_challenge_reward(&self, difficulty: &str, completion_quality: f64) -> i64 {
    ...
    pub async fn ensure_default_badges(&self) -> Result<(), RewardError> {
        let badges = vec![
            ...
            Badge {
                ...
                requirements: serde_json::json!({"quizzes_passed": 10, "min_score": 90}).to_string(),
                is_active: true,
                created_at: chrono::Utc::now(),
            },
            Badge {
                ...
                requirements: serde_json::json!({"category": "trading", "all_courses": true}).to_string(),
                is_active: true,
                created_at: chrono::Utc::now(),
            },
        ];
    ...
    pub async fn get_reward_stats(&self) -> Result<RewardStats, RewardError> {
        let total_rewards_issued: i64 = sqlx::query_scalar(
        ...
        let total_badges_earned: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM earned_badges"
        )
        .fetch_one(&self.pool)
        .await?;

        let total_certificates_issued: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM certificates"
        )
        .fetch_one(&self.pool)
        .await?;

        let unique_reward_earners: i64 = sqlx::query_scalar(
            "SELECT COUNT(DISTINCT wallet_address) FROM rewards"
        )
        .fetch_one(&self.pool)
        .await?;
    ...
    fn reward_from_row(row: &sqlx::sqlite::SqliteRow) -> Result<Reward, RewardError> {
        ...
        Ok(Reward {
            ...
            claimed_at: claimed_str.and_then(|s| DateTime::parse_from_rfc3339(&s).ok().map(|d| d.with_timezone(&Utc))),
        })
    }
