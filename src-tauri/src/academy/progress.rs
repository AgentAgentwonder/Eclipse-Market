...
impl ProgressTracker {
    pub async fn new(app_handle: &AppHandle) -> Result<Self, ProgressError> {
        let app_dir = app_handle
            .path_resolver()
            .app_data_dir()
            .ok_or_else(|| ProgressError::Io(std::io::Error::new(
    ...
    async fn init_schema(pool: &Pool<Sqlite>) -> Result<(), ProgressError> {
        ...
        // Create indexes
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_user_progress_wallet ON user_progress(wallet_address)")
            .execute(pool)
            .await?;
    ...
    pub async fn start_course(&self, wallet_address: &str, course_id: &str) -> Result<UserProgress, ProgressError> {
    ...
    pub async fn get_user_progress(&self, wallet_address: &str, course_id: &str) -> Result<UserProgress, ProgressError> {
        let row = sqlx::query(
            "SELECT * FROM user_progress WHERE wallet_address = ? AND course_id = ?"
        )
        .bind(wallet_address)
        .bind(course_id)
        .fetch_one(&self.pool)
    ...
    pub async fn update_course_progress(&self, wallet_address: &str, course_id: &str, completed_lesson_id: &str) -> Result<(), ProgressError> {
    ...
    pub async fn complete_course(&self, wallet_address: &str, course_id: &str) -> Result<(), ProgressError> {
        sqlx::query(
            r#"
            UPDATE user_progress 
    ...
    pub async fn start_lesson(&self, wallet_address: &str, lesson_id: &str, course_id: &str) -> Result<LessonProgress, ProgressError> {
    ...
    pub async fn get_lesson_progress(&self, wallet_address: &str, lesson_id: &str) -> Result<LessonProgress, ProgressError> {
        let row = sqlx::query(
            "SELECT * FROM lesson_progress WHERE wallet_address = ? AND lesson_id = ?"
        )
        .bind(wallet_address)
        .bind(lesson_id)
        .fetch_one(&self.pool)
    ...
    pub async fn update_lesson_progress(&self, wallet_address: &str, lesson_id: &str, time_spent: i64, last_position: Option<String>) -> Result<(), ProgressError> {
        sqlx::query(
            r#"
    ...
    pub async fn complete_lesson(&self, wallet_address: &str, lesson_id: &str, course_id: &str) -> Result<(), ProgressError> {
        sqlx::query(
            r#"
            UPDATE lesson_progress 
        ...
        self.update_course_progress(wallet_address, course_id, lesson_id).await?;
    ...
    pub async fn get_quiz_attempts(&self, wallet_address: &str, quiz_id: &str) -> Result<Vec<QuizAttempt>, ProgressError> {
        let rows = sqlx::query(
    ...
    pub async fn submit_challenge(&self, submission: ChallengeSubmission) -> Result<ChallengeSubmission, ProgressError> {
        sqlx::query(
            r#"
        ...
        self.update_user_activity(&submission.wallet_address).await?;

        Ok(submission)
    }

    pub async fn get_challenge_submissions(&self, wallet_address: &str) -> Result<Vec<ChallengeSubmission>, ProgressError> {
        let rows = sqlx::query(
    ...
    // Webinar attendance
    pub async fn record_webinar_attendance(&self, attendance: WebinarAttendance) -> Result<WebinarAttendance, ProgressError> {
        sqlx::query(
            r#"
        ...
        self.update_user_activity(&attendance.wallet_address).await?;

        Ok(attendance)
    }

    // Mentor sessions
    pub async fn create_mentor_session(&self, session: MentorSession) -> Result<MentorSession, ProgressError> {
        sqlx::query(
            r#"
    ...
    pub async fn get_user_mentor_sessions(&self, wallet_address: &str) -> Result<Vec<MentorSession>, ProgressError> {
        let rows = sqlx::query(
    ...
    pub async fn get_user_stats(&self, wallet_address: &str) -> Result<UserStats, ProgressError> {
        ...
        let total_courses_enrolled: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM user_progress WHERE wallet_address = ?"
        )
        .bind(wallet_address)
        .fetch_one(&self.pool)
        .await?;
        ...
        let total_webinars_attended: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM webinar_attendance WHERE wallet_address = ?"
        )
        .bind(wallet_address)
        .fetch_one(&self.pool)
        .await?;
        ...
        let row = sqlx::query(
            "SELECT * FROM user_stats WHERE wallet_address = ?"
        )
        .bind(wallet_address)
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn add_badge(&self, wallet_address: &str, badge_id: &str) -> Result<(), ProgressError> {
        ...
        sqlx::query(
            "UPDATE user_stats SET badges_earned = ? WHERE wallet_address = ?"
        )
        .bind(badges_json)
        .bind(wallet_address)
        .execute(&self.pool)
    ...
    pub async fn get_leaderboard(&self, limit: i64) -> Result<Vec<LeaderboardEntry>, ProgressError> {
        let rows = sqlx::query(
            r#"
            SELECT 
                wallet_address,
    ...
    // Helper methods
    fn user_progress_from_row(row: &sqlx::sqlite::SqliteRow) -> Result<UserProgress, ProgressError> {
        ...
        Ok(UserProgress {
            ...
            started_at: DateTime::parse_from_rfc3339(&started_str)
                .map_err(|e| ProgressError::InvalidData(e.to_string()))?
                .with_timezone(&Utc),
            completed_at: completed_str.and_then(|s| DateTime::parse_from_rfc3339(&s).ok().map(|d| d.with_timezone(&Utc))),
        })
    }

    fn lesson_progress_from_row(row: &sqlx::sqlite::SqliteRow) -> Result<LessonProgress, ProgressError> {
        let status_str: String = row.try_get("status")?;
        let status = match status_str.as_str() {
            "notstarted" => ProgressStatus::NotStarted,
            "inprogress" => ProgressStatus::InProgress,
    ...
    fn challenge_submission_from_row(row: &sqlx::sqlite::SqliteRow) -> Result<ChallengeSubmission, ProgressError> {
        ...
        Ok(ChallengeSubmission {
            ...
            submitted_at: DateTime::parse_from_rfc3339(&submitted_str)
                .map_err(|e| ProgressError::InvalidData(e.to_string()))?
                .with_timezone(&Utc),
            reviewed_at: reviewed_str.and_then(|s| DateTime::parse_from_rfc3339(&s).ok().map(|d| d.with_timezone(&Utc))),
        })
    }

    fn mentor_session_from_row(row: &sqlx::sqlite::SqliteRow) -> Result<MentorSession, ProgressError> {
