...
pub async fn complete_course(
    ...
    if let Some(badge_id) = course.badge_id {
        ...
        reward_engine
            .read()
            .await
            .award_badge(&wallet_address, &badge_id, &format!("Completed course: {}", course.title))
            .await
            .map_err(|e| e.to_string())?;
    }
...
pub async fn submit_quiz(
    ...
    if result.passed {
        ...
        let xp = reward_engine
            .read()
            .await
            .calculate_quiz_reward(result.score, result.total_points, result.time_taken_minutes < 30)
            .await;
