...
impl ContentService {
    pub async fn new(app_handle: &AppHandle) -> Result<Self, ContentError> {
        let app_dir = app_handle
            .path_resolver()
            .app_data_dir()
            .ok_or_else(|| ContentError::Io(std::io::Error::new(
    ...
    pub async fn list_courses(&self, category: Option<String>, level: Option<CourseLevel>) -> Result<Vec<Course>, ContentError> {
    ...
    pub async fn list_webinars(&self, status: Option<String>) -> Result<Vec<Webinar>, ContentError> {
        let query = if let Some(s) = status {
            format!("SELECT * FROM webinars WHERE status = '{}' ORDER BY scheduled_at ASC", s)
        } else {
    ...
    pub async fn list_mentors(&self, expertise_area: Option<String>) -> Result<Vec<Mentor>, ContentError> {
        let rows = sqlx::query(
            "SELECT * FROM mentors WHERE is_active = 1 ORDER BY rating DESC"
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn get_stats(&self) -> Result<ContentStats, ContentError> {
        let total_courses: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM courses WHERE is_published = 1")
            .fetch_one(&self.pool)
            .await?;
        ...
        let total_mentors: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM mentors WHERE is_active = 1")
            .fetch_one(&self.pool)
            .await?;
    ...
    fn challenge_from_row(row: &sqlx::sqlite::SqliteRow) -> Result<Challenge, ContentError> {
        ...
        Ok(Challenge {
            ...
            start_date: start_date_str.and_then(|s| DateTime::parse_from_rfc3339(&s).ok().map(|d| d.with_timezone(&Utc))),
