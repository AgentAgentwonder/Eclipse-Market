...
pub async fn submit_signed_vote(
    ...
    let mut guard = manager.write().await;
    guard
        .submit_vote(proposal_id, wallet_address, vote_choice, voting_power, signature)
        .await
        .map_err(|err| err.to_string())
}
...
pub async fn prepare_vote_signature(
    ...
    let message = signature::create_vote_message(&proposal_id, &vote_choice, &wallet_address, timestamp);
...
pub async fn verify_vote_signature(
    ...
    signature::verify_vote_signature(&request, &response)
        .map_err(|err| err.to_string())
}
