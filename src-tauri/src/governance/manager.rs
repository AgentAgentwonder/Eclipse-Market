...
impl GovernanceManager {
    ...
    pub async fn sync_proposals(&mut self, dao_id: &str) -> Result<Vec<GovernanceProposal>, AppError> {
    ...
    pub async fn submit_vote(
        ...
        if let Some(dao_proposals) = self.proposals.values_mut().find(|proposals| {
            proposals.iter().any(|p| p.proposal_id == proposal_id)
        }) {
            if let Some(proposal) = dao_proposals.iter_mut().find(|p| p.proposal_id == proposal_id) {
                match vote_choice {
    ...
    pub async fn revoke_delegation(&mut self, delegation_id: &str, wallet_address: &str) -> Result<(), AppError> {
        if let Some(delegations) = self.delegations.get_mut(wallet_address) {
            if let Some(delegation) = delegations.iter_mut().find(|d| d.delegation_id == delegation_id) {
    ...
    pub async fn analyze_proposal_impact(&self, proposal_id: &str) -> Result<ProposalImpactAnalysis, AppError> {
        ...
        if current_proposal.quorum_required > 0.0 {
            let total_votes = current_proposal.yes_votes + current_proposal.no_votes + current_proposal.abstain_votes;
        ...
        potential_impacts.insert("treasury".to_string(), "May affect DAO treasury balance".to_string());
        potential_impacts.insert("governance".to_string(), "Changes to governance parameters".to_string());
        ...
        Ok(ProposalImpactAnalysis {
            ...
            historical_similarity_score: if similar_proposals.is_empty() {
                ...
                similar_proposals.iter().map(|sp| sp.similarity_score).sum::<f64>() / similar_proposals.len() as f64
            },
            similar_proposals,
            predicted_outcome,
            confidence,
    ...
    pub async fn get_governance_summary(&self, wallet_address: &str) -> GovernanceSummary {
        ...
        let active_proposals = self
            .get_all_active_proposals(wallet_address)
            .await
            .len();
    ...
    async fn fetch_dao_memberships(&self, wallet_address: &str) -> Result<Vec<DAOMembership>, AppError> {
    ...
    fn find_similar_proposals(
        ...
        for proposal in all_proposals {
            ...
            if proposal.title.to_lowercase().split_whitespace().any(|word| {
                target.title.to_lowercase().split_whitespace().any(|tw| tw == word)
            }) {
                similarity_score += 0.3;
            }
    ...
    pub fn find_dao_id_by_proposal(&self, proposal_id: &str) -> Option<String> {
        for (dao_id, proposals) in &self.proposals {
            if proposals.iter().any(|proposal| proposal.proposal_id == proposal_id) {
