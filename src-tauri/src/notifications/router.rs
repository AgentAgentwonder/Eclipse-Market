...
impl NotificationRouter {
    ...
    pub async fn test_telegram(
        ...
        match self.telegram_client.send_message(&config, message, false).await {
            Ok(_) => {
    ...
    pub async fn test_discord(
        ...
        match self.discord_client.send_message(&config, message, false).await {
            Ok(_) => {
                let duration = start.elapsed().as_millis() as u64;
                Ok(TestMessageResult {
                    success: true,
    ...
    async fn send_telegram_alert(
        ...
        match self.telegram_client.send_message(config, &message, true).await {
            Ok(_) => Ok(()),
            Err(e) => {
                let rate_limiter = self.rate_limiter.read().await;
                rate_limiter
    ...
    async fn log_delivery(
        ...
        let (status, error) = match result {
            ...
            Err(NotificationError::RateLimited(e)) => (DeliveryStatus::RateLimited, Some(e.as_str())),
