...
impl EmailManager {
    ...
    pub async fn save_config(&self, config: SmtpConfig, keystore: &Keystore) -> Result<(), EmailError> {
    ...
    pub async fn get_delivery_history(&self, limit: i32) -> Result<Vec<EmailDeliveryRecord>, EmailError> {
        let rows = sqlx::query(
            r#"
    ...
    fn render_template(
        ...
        let text = template.text_body.as_ref()
