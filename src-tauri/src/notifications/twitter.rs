...
impl TwitterManager {
    ...
    pub async fn save_config(&self, config: TwitterConfig, keystore: &Keystore) -> Result<(), TwitterError> {
    ...
    // Keyword management
    pub async fn add_keyword(&self, keyword: String, category: String) -> Result<TwitterSentimentKeyword, TwitterError> {
    ...
    pub async fn add_influencer(&self, username: String, display_name: String) -> Result<TwitterInfluencer, TwitterError> {
        let id = uuid::Uuid::new_v4().to_string();
        let now = Utc::now().to_rfc3339();
    ...
    // Sentiment fetching
    pub async fn fetch_sentiment(&self, keyword: &str, config: &TwitterConfig) -> Result<TwitterSentimentData, TwitterError> {
        if !config.sentiment_tracking_enabled {
            return Err(TwitterError::Internal("Sentiment tracking is disabled".to_string()));
        }
    ...
    fn analyze_tweet_sentiment(&self, text: &str) -> f64 {
        ...
        let negative_keywords = ["bearish", "dump", "bad", "worst", "terrible", "hate", "loss", "crash", "scam", "rug"];
        
        let text_lower = text.to_lowercase();
        let positive = positive_keywords.iter().filter(|&kw| text_lower.contains(kw)).count() as f64;
        let negative = negative_keywords.iter().filter(|&kw| text_lower.contains(kw)).count() as f64;
    ...
    pub async fn get_sentiment_history(&self, keyword: &str, limit: i32) -> Result<Vec<TwitterSentimentData>, TwitterError> {
        let rows = sqlx::query(
            r#"
    ...
    pub async fn get_stats(&self) -> Result<TwitterStats, TwitterError> {
        ...
        let total_sentiment = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM twitter_sentiment_data"
        )
        .fetch_one(&self.pool)
        .await?;

        let tracked_keywords = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM twitter_keywords WHERE enabled = 1"
        )
        .fetch_one(&self.pool)
        .await?;
...
fn twitter_db_path(app: &AppHandle) -> Result<PathBuf, TwitterError> {
    let app_dir = app
        .path_resolver()
        .app_data_dir()
        .ok_or_else(|| TwitterError::Internal("Unable to resolve app data directory".to_string()))?;

    std::fs::create_dir_all(&app_dir)
        .map_err(|e| TwitterError::Internal(format!("Failed to create app data directory: {}", e)))?;
...
#[tauri::command]
pub async fn twitter_remove_keyword(
    id: String,
    app: AppHandle,
) -> Result<String, String> {
    let manager = TwitterManager::new(&app)
...
#[tauri::command]
pub async fn twitter_remove_influencer(
    id: String,
    app: AppHandle,
) -> Result<String, String> {
    let manager = TwitterManager::new(&app)
