...
impl AutoCompoundEngine {
    ...
    pub async fn execute_compound(&self, position: &DeFiPosition) -> Result<CompoundTransaction, String> {
        let settings = match self.get_config(&position.id).await {
            Some(s) => s,
    ...
    pub async fn estimate_apy_boost(&self, position: &DeFiPosition, compound_frequency: u64) -> f64 {
...
#[tauri::command]
pub async fn get_auto_compound_config(position_id: String) -> Result<Option<AutoCompoundSettings>, String> {
    let engine = AutoCompoundEngine::new();
...
#[tauri::command]
pub async fn estimate_compound_apy_boost(position_id: String, apy: f64, value_usd: f64, compound_frequency: u64) -> Result<f64, String> {
    let engine = AutoCompoundEngine::new();
    ...
    Ok(engine.estimate_apy_boost(&position, compound_frequency).await)
}
