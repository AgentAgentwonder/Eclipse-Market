...
impl PositionManager {
        ...
        for position in summary.positions.iter() {
            let risk_level = match position.position_type {
                ...
                PositionType::LiquidityPool | PositionType::Farming => {
                    if position.apy > 30.0 { RiskLevel::High } else { RiskLevel::Medium }
                }
    ...
    pub async fn recommend_auto_compound(&self, wallet: &str) -> Result<Vec<AutoCompoundSettings>, String> {
        let summary = self.build_portfolio_summary(wallet).await?;
...
pub async fn get_defi_portfolio_summary(wallet: String) -> Result<PortfolioSummary, String> {
    PositionManager::new().build_portfolio_summary(&wallet).await
}
...
#[tauri::command]
pub async fn get_auto_compound_recommendations(wallet: String) -> Result<Vec<AutoCompoundSettings>, String> {
    PositionManager::new()
