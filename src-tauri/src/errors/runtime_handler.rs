...
impl RuntimeHandler {
    ...
    fn log_error(
        ...
        *counts.entry(error_code.to_string()).or_insert(0) += 1;
        ...
        self.logger.log(LogLevel::Error, message, Some(&format!("{:?}", category)), Some(error_details), None);
    }
    ...
    fn add_network_recovery_plan(&self, error_code: &str) {
        ...
        self.recovery_plans.write().insert(error_code.to_string(), plan);
    }
    ...
    fn add_database_recovery_plan(&self, error_code: &str) {
        let plan = RecoveryPlan {
            ...
            state_recovery: Some(crate::recovery::StateRecoveryPlan {
                ...
                actions: vec!["Close connection".to_string(), "Reopen connection".to_string()],
            }),
            data_recovery: None,
        };
        self.recovery_plans.write().insert(error_code.to_string(), plan);
