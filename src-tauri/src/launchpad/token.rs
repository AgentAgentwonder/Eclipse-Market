...
impl TokenManager {
    ...
    pub async fn simulate_token_creation(
        ...
        if request.mint_authority_enabled {
            warnings.push(
                "Mint authority enabled - tokens can be minted after launch".to_string(),
            );
        }

        if request.freeze_authority_enabled {
            warnings.push(
                "Freeze authority enabled - accounts can be frozen".to_string(),
            );
        }
    ...
    fn validate_token_request(&self, request: &CreateTokenRequest) -> Result<(), AppError> {
        ...
        if request.decimals > 9 {
            return Err(AppError::Generic(
                "Decimals cannot exceed 9".to_string(),
            ));
        }
