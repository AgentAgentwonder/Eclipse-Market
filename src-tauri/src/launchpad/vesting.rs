...
impl VestingManager {
    ...
    pub fn release_tokens(&self, schedule_id: &str, amount: u64) -> Result<VestingSchedule, AppError> {
        if amount == 0 {
            return Err(AppError::Validation("Release amount must be greater than 0".to_string()));
        }
        ...
        if amount > releasable {
            return Err(AppError::Validation("Requested amount exceeds releasable tokens".to_string()));
        }
    ...
    fn releasable_linear(&self, schedule: &VestingSchedule, now: DateTime<Utc>) -> u64 {
        ...
        let vested = (schedule.total_amount as u128 * elapsed_seconds as u128)
            / total_duration as u128;

        vested
            .saturating_sub(schedule.released_amount as u128)
            .min(schedule.total_amount as u128)
            as u64
    }
