...
impl LiquidityLocker {
    ...
    pub async fn create_lock(
        ...
        // Validate amounts
        if request.amount == 0 {
            return Err(AppError::Validation("Lock amount must be greater than 0".to_string()));
        }

        if request.duration_seconds < 86400 {
            // At least 1 day
            return Err(AppError::Validation("Lock duration must be at least 1 day".to_string()));
        }

        // Get authority keypair from keystore
        let keystore: tauri::State<Keystore> = app.state();
    ...
    pub async fn unlock_liquidity(
        ...
        if Utc::now() < lock.unlock_date {
            return Err(AppError::Validation("Lock period has not expired yet".to_string()));
        }
