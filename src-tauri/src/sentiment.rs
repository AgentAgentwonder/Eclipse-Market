...
impl SentimentManager {
    ...
    pub fn add_sentiment_data(&mut self, token_address: String, posts: Vec<SocialPost>) {
        ...
        let avg_score = if total > 0.0 { total_score / total } else { 0.0 };
        ...
        let token_sentiment = self.token_sentiments
        ...
        if token_sentiment.trend.len() > 100 {
            token_sentiment.trend.drain(0..token_sentiment.trend.len() - 100);
        }
...
pub fn analyze_sentiment(text: &str) -> SentimentResult {
    let positive_words = [
        ...
        "amazing", "best", "love", "high", "rally", "boom",
    ];
    let negative_words = [
...
#[tauri::command]
pub async fn fetch_social_mentions(
    token_address: String,
