...
impl WebhookManager {
    ...
    fn webhooks_db_path(app: &AppHandle) -> Result<std::path::PathBuf, WebhookError> {
        let mut path = app
            .path_resolver()
            .app_data_dir()
            .ok_or_else(|| WebhookError::Internal("Unable to resolve app data directory".to_string()))?;
    ...
    pub async fn create_webhook(&self, mut config: WebhookConfig) -> Result<WebhookConfig, WebhookError> {
        let now = Utc::now();
    ...
    pub async fn update_webhook(&self, id: &str, config: WebhookConfig) -> Result<(), WebhookError> {
    ...
    fn row_to_config(&self, row: sqlx::sqlite::SqliteRow) -> Result<WebhookConfig, WebhookError> {
        ...
        Ok(WebhookConfig {
            ...
            variables: serde_json::from_str(&variables_json).map_err(WebhookError::Serialization)?,
    ...
    pub async fn test_webhook(
        ...
        let config = self.get_webhook(id).await?;
        let result = self
            .send_once(&config, variables, true, 1)
            .await?;
