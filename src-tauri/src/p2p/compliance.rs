...
impl ComplianceChecker {
        ...
        if let Some(reputation) = creator_reputation {
            ...
            if reputation.is_blacklisted {
                errors.push(format!(
                    ...
                    reputation.blacklist_reason.as_ref().unwrap_or(&"Unknown reason".to_string())
                ));
    ...
    pub fn generate_safety_warnings(&self, risk_level: &str) -> Vec<String> {
        ...
        warnings.push("⚠️ Use the built-in chat to maintain a record of all communications".to_string());

        match risk_level {
            "high" | "critical" => {
                warnings.push("🚨 HIGH RISK: This trade has significant risk factors".to_string());
                warnings.push("🚨 Consider cancelling or requesting additional verification".to_string());
