...
impl P2PDatabase {
    ...
    pub async fn create_escrow(&self, request: CreateEscrowRequest) -> Result<Escrow> {
        let offer = self.get_offer(&request.offer_id).await?
    ...
    pub async fn get_dispute_evidence(&self, dispute_id: &str) -> Result<Vec<DisputeEvidence>> {
        ...
        for row in rows {
            ...
            evidence_list.push(DisputeEvidence {
                ...
                attachments,
                submitted_at: DateTime::parse_from_rfc3339(&row.try_get::<String, _>("submitted_at")?)
                    .map(|dt| dt.with_timezone(&Utc))?,
            });
        }
    ...
    pub async fn update_trader_stats(
        ...
        let new_successful = if successful { profile.successful_trades + 1 } else { profile.successful_trades };
        let new_cancelled = if cancelled { profile.cancelled_trades + 1 } else { profile.cancelled_trades };
        let new_disputed = if disputed { profile.disputed_trades + 1 } else { profile.disputed_trades };
        ...
        let reputation_score = 50.0 + (success_rate * 40.0) - (dispute_rate * 30.0) - (cancel_rate * 20.0);
        let reputation_score = reputation_score.max(0.0).min(100.0);

        let now = Utc::now().to_rfc3339();
        let first_trade = profile.first_trade_at.map(|dt| dt.to_rfc3339()).unwrap_or_else(|| now.clone());
    ...
    pub async fn get_stats(&self) -> Result<P2PStats> {
        ...
        let active_offers: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM p2p_offers WHERE is_active = 1")
            .fetch_one(&self.pool)
            .await?;
        ...
        let completed_escrows: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM p2p_escrows WHERE state = 'completed'"
        )
        .fetch_one(&self.pool)
        .await?;
