...
impl ComprehensiveLogger {
    pub fn new(app: &AppHandle) -> Result<Self, std::io::Error> {
        let mut log_dir = app
            .path_resolver()
            .app_data_dir()
            .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "App data dir not found"))?;
    ...
    pub fn performance(&self, message: &str, duration_ms: f64, details: Option<serde_json::Value>) {
        ...
        self.log(LogLevel::Performance, message, None, Some(d), Some(duration_ms));
    }
    ...
    fn log_to_console(&self, entry: &LogEntry, colored: bool) {
        ...
        let category_str = entry.category.as_ref().map(|c| format!(" [{}]", c)).unwrap_or_default();
        
        let details_str = entry.details.as_ref().map(|d| {
            format!(" | {}", serde_json::to_string(d).unwrap_or_default())
        }).unwrap_or_default();
    ...
    fn format_log_entry(&self, entry: &LogEntry) -> String {
        ...
        let category = entry.category.as_ref().map(|c| format!(" [{}]", c)).unwrap_or_default();
        let details = entry.details.as_ref().map(|d| {
            format!(" | {}", serde_json::to_string(d).unwrap_or_default())
        }).unwrap_or_default();
        
        format!("{} {} {}{}{}", timestamp, level, entry.message, category, details)
    }
