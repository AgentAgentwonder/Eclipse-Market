...
impl AIPortfolioAdvisor {
    ...
    pub async fn get_risk_profile(&self, id: &str) -> Result<Option<UserRiskProfile>, sqlx::Error> {
        ...
        if let Some(row) = row {
            ...
            let custom_settings: Option<CustomRiskSettings> = custom_settings_json
    ...
    pub async fn generate_recommendation(
        ...
        for (symbol, target_pct) in target_allocations.iter() {
            ...
            let reasoning = if action == "buy" {
                ...
                format!("Current allocation of {:.1}% is within target range", current_pct)
            };

            let mint = current_position
                .map(|p| p.mint.clone())
                .unwrap_or_default();
        ...
        let expected_return = self.calculate_expected_return(&target_allocations, risk_profile.profile.as_str());
        let expected_risk = self.calculate_expected_risk(&target_allocations, risk_profile.profile.as_str());
    ...
    fn calculate_optimal_allocations(
        ...
        for position in positions {
            ...
            let performance_factor = if position.unrealized_pnl > 0.0 { 1.1 } else { 0.9 };
            ...
            let allocation = (base_allocation * performance_factor * (1.0 + adjustment) * risk_tolerance)
                .max(5.0)
                .min(40.0);
    ...
    fn calculate_expected_return(&self, allocations: &HashMap<String, f64>, risk_profile: &str) -> f64 {
    ...
    fn calculate_expected_risk(&self, allocations: &HashMap<String, f64>, risk_profile: &str) -> f64 {
        ...
        let concentration_penalty = if allocations.len() < 3 {
            3.0
        } else {
            0.0
        };
    ...
    pub async fn save_recommendation(
        ...
        let allocations_json = serde_json::to_string(&recommendation.allocations).unwrap_or_default();
...
mod tests {
    ...
    fn test_calculate_diversification_score() {
        ...
        assert!(allocations.len() > allocations_concentrated.len() || 
                allocations.values().cloned().fold(0.0, f64::max) < 
                allocations_concentrated.values().cloned().fold(0.0, f64::max));
    }
}
...
pub async fn save_risk_profile(
    ...
    advisor.save_risk_profile(profile).await
...
pub async fn get_risk_profile(
    ...
    let advisor = advisor.read().await;
    advisor.get_risk_profile(&id).await
...
pub async fn get_portfolio_recommendations(
    ...
    let advisor = advisor.read().await;
    advisor.get_recommendations(limit).await
...
pub async fn apply_portfolio_recommendation(
    ...
    let advisor = advisor.read().await;
    advisor.apply_recommendation(&recommendation_id).await
...
pub async fn generate_weekly_portfolio_update(
    ...
    let advisor = advisor.read().await;
    advisor.generate_weekly_update(portfolio_value, weekly_return, positions, risk_profile).await
}
...
pub async fn get_weekly_portfolio_updates(
    ...
    let advisor = advisor.read().await;
    advisor.get_weekly_updates(limit).await
...
pub async fn get_performance_history(
    ...
    let advisor = advisor.read().await;
    advisor.get_performance_history(recommendation_id.as_deref(), limit).await
