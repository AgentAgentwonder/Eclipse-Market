...
impl WatchlistManager {
    ...
    pub async fn add_item(
        ...
        let max_position: Option<i32> = sqlx::query_scalar(
            "SELECT MAX(position) FROM watchlist_items WHERE watchlist_id = ?1",
        )
        .bind(watchlist_id)
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn remove_item(
        ...
        let result = sqlx::query(
            "DELETE FROM watchlist_items WHERE watchlist_id = ?1 AND mint = ?2",
        )
        .bind(watchlist_id)
        .bind(mint)
        .execute(&self.pool)
    ...
    async fn get_watchlist_items(&self, watchlist_id: &str) -> Result<Vec<WatchlistItem>, WatchlistError> {
        let rows = sqlx::query(
            r#"
...
fn watchlist_db_path(app: &AppHandle) -> Result<PathBuf, WatchlistError> {
    let app_data_dir = app
        .path_resolver()
        .app_data_dir()
        .ok_or_else(|| WatchlistError::Internal("Unable to resolve app data directory".to_string()))?;
