...
use super::polymarket_adapter::{PolymarketAdapter, PolymarketMarket, generate_mock_polymarket_markets};
use super::drift_adapter::{DriftAdapter, DriftPrediction, generate_mock_drift_predictions};
...
impl PredictionMarketService {
    ...
    fn normalize_polymarket(market: &PolymarketMarket) -> PredictionMarket {
        let end_timestamp = market.end_date.as_ref()
        ...
        PredictionMarket {
            ...
            category: market.tags.first().cloned().unwrap_or_else(|| "General".to_string()),
    ...
    pub async fn fetch_all_markets(&self, use_mock: bool) -> Result<Vec<PredictionMarket>, String> {
        ...
        let polymarket_markets = if use_mock {
            ...
            self.polymarket_adapter.fetch_markets().await.unwrap_or_else(|_| generate_mock_polymarket_markets())
        };
        ...
        let drift_predictions = if use_mock {
            ...
            self.drift_adapter.fetch_predictions().await.unwrap_or_else(|_| generate_mock_drift_predictions())
        };
    ...
    pub async fn search_markets(&self, query: &str, use_mock: bool) -> Result<Vec<PredictionMarket>, String> {
    ...
    pub async fn create_custom_prediction(&self, prediction: CustomPrediction) -> Result<CustomPrediction, String> {
        let mut predictions = self.custom_predictions.write().await;
        predictions.push(prediction.clone());
        Ok(prediction)
    }

    pub async fn get_custom_predictions(&self, user_id: &str) -> Result<Vec<CustomPrediction>, String> {
        let predictions = self.custom_predictions.read().await;
        Ok(predictions.iter().filter(|p| p.user_id == user_id).cloned().collect())
    }

    pub async fn update_custom_prediction(&self, id: &str, updated: CustomPrediction) -> Result<CustomPrediction, String> {
        let mut predictions = self.custom_predictions.write().await;
    ...
    pub async fn record_performance(&self, performance: PredictionPerformance) -> Result<(), String> {
        let mut performances = self.performances.write().await;
        performances.push(performance);
        Ok(())
    }

    pub async fn get_portfolio_comparison(&self, user_id: &str) -> Result<PortfolioComparison, String> {
    ...
    pub async fn calculate_consensus(&self, market_id: &str, use_mock: bool) -> Result<ConsensusData, String> {
