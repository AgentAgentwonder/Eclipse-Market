...
impl PushNotificationManager {
    ...
    fn queue_notification(&mut self, notification: PushNotification) {
        ...
        self.notifications.insert(notification.notification_id.clone(), notification);
    }
    ...
    pub fn get_pending_notifications(&self, device_id: &str) -> Vec<PushNotification> {
        self.queue
            ...
            .filter(|notification| notification.device_id == device_id && notification.delivered_at.is_none())
            .cloned()
            .collect()
    }
    ...
    pub fn dequeue_next(&mut self, device_id: &str) -> Option<PushNotification> {
        let position = self
            .queue
            .iter()
            .position(|id| self.notifications.get(id).map(|n| n.device_id.as_str() == device_id).unwrap_or(false));
