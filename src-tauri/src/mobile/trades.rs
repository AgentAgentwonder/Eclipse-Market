...
impl MobileTradeEngine {
    ...
    pub async fn execute_quick_trade(
        ...
        let session = {
            ...
            auth.authenticate_session(trade.session_token.clone()).await?
        };
    ...
    fn enforce_safety_checks(&self, _session: &MobileSession, trade: &QuickTradeRequest) -> Result<()> {
        let checks = vec![SafetyCheck::MaxNotionalValue(50_000.0), SafetyCheck::MaxOrderSize(1_000.0)];
