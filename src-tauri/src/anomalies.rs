...
impl AnomalyDetector {
    ...
    pub fn add_price_data(&mut self, token_address: String, data: PriceData) {
        let history = self.price_history.entry(token_address.clone()).or_insert_with(Vec::new);
        history.push(data);
    ...
    pub fn add_transaction_data(&mut self, token_address: String, data: TransactionData) {
        let history = self.transaction_history.entry(token_address.clone()).or_insert_with(Vec::new);
        history.push(data);
    ...
    fn detect_zscore_anomaly(&mut self, token_address: &str, history: &[PriceData], latest: &PriceData) {
    ...
    fn detect_iqr_anomaly(&mut self, token_address: &str, history: &[PriceData], latest: &PriceData) {
        ...
        if q3_idx < prices.len() {
            ...
            if latest.price < lower_bound || latest.price > upper_bound {
                ...
                let direction = if latest.price < lower_bound { "below" } else { "above" };
                let bound = if latest.price < lower_bound { lower_bound } else { upper_bound };
    ...
    fn detect_volume_anomaly(&mut self, token_address: &str, history: &[PriceData], latest: &PriceData) {
        ...
        if mean_volume > 0.0 {
            ...
            if volume_ratio >= 5.0 {
                let severity = if volume_ratio >= 10.0 { "high" } else { "medium" };
                ...
                details.insert("current_volume".to_string(), format!("{:.2}", latest.volume));
    ...
    fn detect_wash_trading(&mut self, token_address: &str) {
        if let Some(history) = self.transaction_history.get(token_address) {
            ...
            let mut address_pairs: HashMap<(String, String), Vec<&TransactionData>> = HashMap::new();
            ...
            for ((addr1, addr2), txs) in address_pairs.iter() {
                if txs.len() >= 3 {
                    ...
                    if back_and_forth >= self.config.wash_trading_threshold {
                        ...
                        let avg_price: f64 = txs.iter().map(|tx| tx.price).sum::<f64>() / txs.len() as f64;
                        ...
                        details.insert("pattern_score".to_string(), format!("{:.2}", back_and_forth));
    ...
    pub fn get_anomalies(&self, token_address: Option<&str>, anomaly_type: Option<&str>) -> Vec<Anomaly> {
        self.anomalies
            .iter()
            .filter(|a| {
    ...
    pub fn get_statistics(&self, token_address: &str) -> Option<AnomalyStatistics> {
        let token_anomalies: Vec<&Anomaly> = self.anomalies
            .iter()
        ...
        let by_type: HashMap<String, usize> = token_anomalies
            .iter()
        ...
        let by_severity: HashMap<String, usize> = token_anomalies
            .iter()
            .fold(HashMap::new(), |mut acc, a| {
...
mod tests {
    ...
    fn test_wash_trading_detection() {
        ...
        for i in 0..10 {
            let from = if i % 2 == 0 { addr1.clone() } else { addr2.clone() };
            let to = if i % 2 == 0 { addr2.clone() } else { addr1.clone() };
