...
fn detect_ping_pong_pattern(edges: &[TokenFlowEdge]) -> Vec<WashTradingPattern> {
    ...
    for ((wallet1, wallet2), pair_edges) in wallet_pairs {
        ...
        if confidence > 0.7 && forward >= WASH_TRADING_MIN_CYCLES && backward >= WASH_TRADING_MIN_CYCLES {
...
fn check_time_windows(edges: &[&TokenFlowEdge]) -> usize {
    ...
    while i < sorted_edges.len() {
        ...
        while j < sorted_edges.len() && sorted_edges[j].timestamp - window_start <= PING_PONG_TIME_WINDOW {
            count += 1;
            j += 1;
        }
...
fn detect_circular_wash_trading(edges: &[TokenFlowEdge]) -> Vec<WashTradingPattern> {
    ...
    let graph = TransactionGraph::from_transactions(
        edges.iter().map(|e| TokenTransaction {
...
pub fn generate_alerts_from_patterns(
    ...
    for flow in circular_flows {
        ...
        metadata.insert(
            ...
            serde_json::json!(flow.confidence),
        );
        metadata.insert(
            "cycles".to_string(),
            serde_json::json!(flow.cycles),
        );
        metadata.insert(
            "amount".to_string(),
            serde_json::json!(flow.amount),
        );
    ...
    for pattern in wash_trading {
        ...
        metadata.insert(
            "pattern".to_string(),
            serde_json::json!(pattern_name),
        );
        metadata.insert(
            "volume".to_string(),
            serde_json::json!(pattern.volume),
        );
    ...
    fn test_ping_pong_detection() {
        ...
        for i in 0..6 {
            edges.push(TokenFlowEdge {
                id: format!("tx{}", i),
                source: if i % 2 == 0 { "A".to_string() } else { "B".to_string() },
                target: if i % 2 == 0 { "B".to_string() } else { "A".to_string() },
                amount: 100.0,
