...
impl TransactionGraph {
    ...
    pub fn add_edge(&mut self, edge: TokenFlowEdge) {
        ...
        self.nodes.entry(edge.source.clone()).or_insert_with(|| {
            TokenFlowNode {
        ...
        self.nodes.entry(edge.target.clone()).or_insert_with(|| {
            TokenFlowNode {
    ...
    pub fn find_paths(&self, start: &str, end: &str, max_depth: usize) -> Vec<Vec<String>> {
        ...
        self.dfs_paths(start, end, max_depth, &mut current_path, &mut visited, &mut paths);
        paths
    }
        ...
        if let Some(neighbors) = self.adjacency.get(current) {
            for neighbor in neighbors {
                if !visited.contains(neighbor) {
                    ...
                    self.dfs_paths(neighbor, end, depth_remaining - 1, current_path, visited, paths);
                    current_path.pop();
    ...
    pub fn detect_cycles(&self) -> Vec<Vec<String>> {
        ...
        for node in self.nodes.keys() {
            if !visited.contains(node) {
                self.find_cycles_util(node, &mut visited, &mut rec_stack, &mut Vec::new(), &mut cycles);
            }
        }
...
pub fn generate_sankey_data(graph: &TokenFlowGraph) -> SankeyData {
    ...
    for (idx, node) in graph.nodes.iter().enumerate() {
        nodes.push(SankeyNode {
            id: node.id.clone(),
            name: node.label.clone().unwrap_or_else(|| truncate_address(&node.address)),
        });
