    ...
    let best_community = community_weights
        ...
        .map(|(community, weight)| {
            let delta = modularity_gain(
                state.community_weight.get(community).copied().unwrap_or(0.0),
                *weight,
                node_weight,
                state.total_weight,
            );
...
fn modularity_gain(
    ...
    (edge_weight / total_weight) - ((community_weight * node_weight) / (2.0 * total_weight * total_weight))
}
...
pub fn build_wallet_clusters(
    ...
    for node in &graph.nodes {
        if let Some(cluster_id) = cluster_map.get(&node.id) {
            let cluster = clusters.entry(*cluster_id).or_insert_with(|| WalletCluster {
    ...
    for edge in &graph.edges {
        if let (Some(source_cluster), Some(target_cluster)) = (
            cluster_map.get(&edge.source),
            cluster_map.get(&edge.target),
        ) {
...
pub fn detect_cluster_performance(clusters: &mut [WalletCluster]) {
    for cluster in clusters.iter_mut() {
        ...
        if cluster.performance.total_pnl > 0.0 {
            cluster.performance.distribution_pattern = if cluster.performance.total_pnl > volume * 0.02 {
