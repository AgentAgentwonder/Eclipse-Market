...
impl WalletOperationsManager {
    ...
    pub fn persist_token_cache(&self, keystore: &Keystore) -> Result<(), KeystoreError> {
        let guard = self.token_cache.lock().map_err(|_| KeystoreError::LockError)?;
    ...
    pub fn persist_address_book(&self, keystore: &Keystore) -> Result<(), KeystoreError> {
        let guard = self.address_book.lock().map_err(|_| KeystoreError::LockError)?;
        let data = serde_json::to_vec(&*guard).map_err(|_| KeystoreError::SerializationError)?;
    ...
    pub fn persist_swap_history(&self, keystore: &Keystore) -> Result<(), KeystoreError> {
        let guard = self.swap_history.lock().map_err(|_| KeystoreError::LockError)?;
        let data = serde_json::to_vec(&*guard).map_err(|_| KeystoreError::SerializationError)?;
...
pub async fn wallet_get_token_balances(
    ...
    if should_refresh {
        ...
        cache.balances.insert(address.clone(), mock_balances.clone());
        cache.last_updated = now;
        
        operations.persist_token_cache(&keystore).map_err(|e| e.to_string())?;
...
pub async fn wallet_estimate_fee(
    ...
    let base_fee = if token_mint.is_some() { 0.00001 } else { 0.000005 };
...
#[tauri::command]
pub async fn wallet_generate_qr(
    data: QRCodeData,
) -> Result<String, String> {
    ...
    Ok(format!("data:image/png;base64,mock_qr_code_for_{}", json_data))
}
...
pub async fn address_book_add_contact(
    ...
    operations.persist_address_book(&keystore).map_err(|e| e.to_string())?;
...
pub async fn address_book_update_contact(
    ...
    let contact = book.contacts.get_mut(&request.contact_id)
...
pub async fn address_book_delete_contact(
    ...
    book.contacts.remove(&contact_id)
        .ok_or_else(|| "Contact not found".to_string())?;
...
pub async fn address_book_list_contacts(
    ...
    contacts.sort_by(|a, b| b.last_used.cmp(&a.last_used).then(b.created_at.cmp(&a.created_at)));
...
pub async fn address_book_search_contacts(
    ...
    let contacts: Vec<AddressBookContact> = book.contacts.values()
        .filter(|c| {
...
pub async fn swap_history_add_entry(
    ...
    operations.persist_swap_history(&keystore).map_err(|e| e.to_string())?;
...
pub async fn swap_history_get_recent(
    ...
    let swaps: Vec<SwapHistoryEntry> = history.swaps
        .iter()
        .take(limit)
        .cloned()
        .collect();
