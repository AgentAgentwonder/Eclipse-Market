...
pub async fn ledger_register_device(
    ...
    let mut devices_guard = state.devices.lock().map_err(|e| {
        LedgerError::Internal(format!("Failed to lock devices: {}", e))
    })?;

    if let Some(existing) = devices_guard.iter_mut().find(|d| d.device_id == device.device_id) {
    ...
    let mut active_id = state.active_device_id.lock().map_err(|e| {
        LedgerError::Internal(format!("Failed to lock active device ID: {}", e))
    })?;
...
pub async fn ledger_validate_transaction(
    ...
    let transaction_bytes = BASE64_ENGINE
        ...
        .map_err(|e| {
            LedgerError::Internal(format!("Invalid transaction encoding: {}", e))
        })?;
...
pub async fn ledger_get_active_device(
    ...
    let active_id_guard = state.active_device_id.lock().map_err(|e| {
        LedgerError::Internal(format!("Failed to lock active device ID: {}", e))
    })?;
...
#[tauri::command]
pub async fn ledger_clear_devices(
    state: State<'_, LedgerState>,
) -> Result<(), LedgerError> {
    let mut devices_guard = state.devices.lock().map_err(|e| {
        LedgerError::Internal(format!("Failed to lock devices: {}", e))
...
mod tests {
    ...
    async fn test_ledger_connect_disconnect() {
        ...
        let connected = ledger_connect_device("test-device-1".to_string(), state.clone()).await.unwrap();
        ...
        ledger_disconnect_device("test-device-1".to_string(), state.clone()).await.unwrap();
        
        let device = ledger_get_device("test-device-1".to_string(), state).await.unwrap();
