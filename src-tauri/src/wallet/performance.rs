...
impl PerformanceDatabase {
    ...
    pub async fn record_trade(&self, request: RecordTradeRequest) -> Result<Trade, sqlx::Error> {
        ...
        let (pnl, hold_duration) = if request.side == "sell" {
            self.calculate_pnl(&request.wallet_address, &request.token_mint, request.price, request.amount, &timestamp).await?
        } else {
            (None, None)
        };
    ...
    async fn calculate_pnl(
        ...
        if let Some(buy) = buy_trade {
            ...
            let duration = sell_timestamp.signed_duration_since(buy.timestamp).num_seconds();
    ...
    pub async fn calculate_performance_score(&self, wallet_address: &str) -> Result<PerformanceScore, sqlx::Error> {
        ...
        let winning_trades = completed_trades.iter().filter(|t| t.pnl.unwrap_or(0.0) > 0.0).count() as i64;
        let losing_trades = completed_trades.iter().filter(|t| t.pnl.unwrap_or(0.0) < 0.0).count() as i64;
        ...
        let profits: Vec<f64> = completed_trades.iter().filter_map(|t| {
            let pnl = t.pnl.unwrap_or(0.0);
            if pnl > 0.0 { Some(pnl) } else { None }
        }).collect();

        let losses: Vec<f64> = completed_trades.iter().filter_map(|t| {
            let pnl = t.pnl.unwrap_or(0.0);
            if pnl < 0.0 { Some(pnl) } else { None }
        }).collect();
        ...
        let avg_hold_duration = if !completed_trades.is_empty() {
            completed_trades.iter()
                ...
                .sum::<i64>() as f64 / completed_trades.len() as f64
        } else {
            0.0
        };
    ...
    fn calculate_overall_score(
        ...
        let score = win_rate_component + profit_component + sharpe_component + consistency_component;
    ...
    async fn check_and_create_alert(&self, wallet_address: &str, new_score: f64) -> Result<(), sqlx::Error> {
    ...
    pub async fn get_score_history(&self, wallet_address: &str, limit: i64) -> Result<Vec<PerformanceScore>, sqlx::Error> {
    ...
    pub async fn get_token_performance(&self, wallet_address: &str) -> Result<Vec<TokenPerformance>, sqlx::Error> {
        let rows = sqlx::query(
            r#"
            SELECT 
                token_mint,
        ...
        for row in rows {
            ...
            performances.push(TokenPerformance {
                ...
                avg_hold_duration_seconds: row.get::<Option<f64>, _>("avg_hold_duration").unwrap_or(0.0),
    ...
    pub async fn get_best_worst_trades(&self, wallet_address: &str, limit: i64) -> Result<BestWorstTrades, sqlx::Error> {
    ...
    pub async fn get_timing_analysis(&self, wallet_address: &str) -> Result<Vec<TimingAnalysis>, sqlx::Error> {
    ...
    pub async fn get_benchmark_comparison(&self, wallet_address: &str) -> Result<Option<BenchmarkComparison>, sqlx::Error> {
    ...
    pub async fn get_score_alerts(&self, wallet_address: &str, limit: i64) -> Result<Vec<ScoreAlert>, sqlx::Error> {
    ...
    pub async fn get_wallet_performance(&self, wallet_address: &str) -> Result<WalletPerformanceData, sqlx::Error> {
...
pub async fn calculate_wallet_performance(
    ...
    let db = db.read().await;
    db.calculate_performance_score(&wallet_address).await.map_err(|e| e.to_string())
}
...
pub async fn get_wallet_performance_data(
    ...
    let db = db.read().await;
    db.get_wallet_performance(&wallet_address).await.map_err(|e| e.to_string())
}
...
pub async fn get_performance_score_history(
    ...
    let db = db.read().await;
    db.get_score_history(&wallet_address, limit).await.map_err(|e| e.to_string())
}
...
pub async fn get_token_performance_breakdown(
    ...
    let db = db.read().await;
    db.get_token_performance(&wallet_address).await.map_err(|e| e.to_string())
}
...
pub async fn get_timing_analysis_data(
    ...
    let db = db.read().await;
    db.get_timing_analysis(&wallet_address).await.map_err(|e| e.to_string())
}
...
pub async fn get_best_worst_trades_data(
    ...
    let db = db.read().await;
    db.get_best_worst_trades(&wallet_address, limit).await.map_err(|e| e.to_string())
}
...
pub async fn get_benchmark_comparison_data(
    ...
    let db = db.read().await;
    db.get_benchmark_comparison(&wallet_address).await.map_err(|e| e.to_string())
}
...
pub async fn get_performance_alerts(
    ...
    let db = db.read().await;
    db.get_score_alerts(&wallet_address, limit).await.map_err(|e| e.to_string())
}
