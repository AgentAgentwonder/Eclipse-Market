...
impl AutoTradingEngine {
    ...
    pub fn calculate_position_size(
        ...
        let size = match strategy.position_sizing.method.as_str() {
            ...
            "volatility_based" => {
                ...
                let position_value = available_capital
                    * strategy.risk_controls.max_position_size
                    / 100.0
                    * scaling_factor;
                position_value / current_price
            }
    ...
    pub fn check_risk_controls(
        ...
        let open_positions = self.positions.get(strategy_id).map(|p| p.len()).unwrap_or(0);
...
pub fn register_auto_trading_state(app: &tauri::App) {
    let engine: SharedAutoTradingEngine = Arc::new(Mutex::new(AutoTradingEngine::new(
