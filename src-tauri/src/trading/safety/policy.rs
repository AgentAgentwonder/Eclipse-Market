...
impl PolicyEngine {
    ...
    pub fn check_trade_policy(
        ...
        if let Some(max_daily) = self.policy.max_daily_trades {
            let count = self.daily_trade_counts.get(wallet_address).copied().unwrap_or(0);
            if count >= max_daily {
                result.add_violation(PolicyViolation {
    ...
    pub fn increment_daily_trade_count(&mut self, wallet_address: &str) {
        let count = self.daily_trade_counts.entry(wallet_address.to_string()).or_insert(0);
        *count += 1;
    }
    ...
    pub fn get_trade_count(&self, wallet_address: &str) -> u32 {
        self.daily_trade_counts.get(wallet_address).copied().unwrap_or(0)
    }
}
