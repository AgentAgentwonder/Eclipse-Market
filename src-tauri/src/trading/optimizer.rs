...
fn crossover(parent1: &HashMap<String, f64>, parent2: &HashMap<String, f64>, crossover_rate: f64) -> HashMap<String, f64> {
...
async fn run_genetic_algorithm(
    ...
    for generation in 0..generations {
            ...
            if let Some(run) = state.runs.get_mut(&run_id) {
                let mut results: Vec<OptimizationResult> = population
                    ...
                    .filter_map(|candidate| {
                        candidate.metrics.as_ref().map(|metrics| OptimizationResult {
        ...
        while new_population.len() < population_size as usize {
            ...
            let mut child_params = crossover(&parent1.parameters, &parent2.parameters, crossover_rate);
            for param in &config.parameters {
                if let Some(value) = child_params.get(&param.name).copied() {
                    let mutated = mutate_parameter(value, param.min, param.max, param.step, mutation_rate);
