...
impl BacktestEngine {
    ...
    pub fn calculate_metrics(&self) -> BacktestMetrics {
        ...
        let final_equity = self.equity_curve.last().map(|e| e.equity).unwrap_or(initial_capital);
        ...
        let winning_trades: Vec<&Trade> = self.trades
            .windows(2)
            .filter_map(|w| {
                if w[0].side == "buy" && w[1].side == "sell" {
                    let pnl = (w[1].price - w[0].price) * w[0].quantity - w[0].commission - w[1].commission;
                    if pnl > 0.0 { Some(&w[1]) } else { None }
                } else {
                    None
                }
            })
        ...
        let losing_trades: Vec<&Trade> = self.trades
            .windows(2)
            .filter_map(|w| {
                if w[0].side == "buy" && w[1].side == "sell" {
                    let pnl = (w[1].price - w[0].price) * w[0].quantity - w[0].commission - w[1].commission;
        ...
        let total_loss = self.trades.windows(2).fold(0.0, |acc, w| {
            if w[0].side == "buy" && w[1].side == "sell" {
                let pnl = (w[1].price - w[0].price) * w[0].quantity - w[0].commission - w[1].commission;
                if pnl < 0.0 { acc + pnl.abs() } else { acc }
            } else {
                acc
            }
        });
        ...
        let profit_factor = if total_loss > 0.0 {
            ...
            if total_win > 0.0 { f64::INFINITY } else { 0.0 }
        };
        ...
        let downside_variance = if downside_returns.len() > 1 {
            downside_returns.iter().map(|r| r.powi(2)).sum::<f64>() / (downside_returns.len() - 1) as f64
        } else {
            0.0
        };
        ...
        let max_drawdown = self.equity_curve.iter().map(|e| e.drawdown).fold(0.0, f64::max);
        let max_drawdown_percent = self.equity_curve.iter().map(|e| e.drawdown_percent).fold(0.0, f64::max);

        // Calculate average trade duration
        let trade_durations: Vec<i64> = self.trades
            .windows(2)
            .filter_map(|w| {
                if w[0].side == "buy" && w[1].side == "sell" {
...
pub async fn backtest_run(config: BacktestConfig) -> Result<BacktestResult, String> {
    ...
    for (i, data) in historical_data.iter().enumerate() {
        ...
        if prev_short_ma >= prev_long_ma && short_ma < long_ma {
            engine.execute_sell(data.timestamp, data.close, Some("MA_CROSS_DOWN".to_string()));
        }
    ...
    if engine.current_position.is_some() {
        ...
        engine.execute_sell(last_data.timestamp, last_data.close, Some("END_OF_PERIOD".to_string()));
    }
