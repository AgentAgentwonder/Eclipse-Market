...
impl PaperTradingDatabase {
    ...
    pub async fn get_or_create_account(&self, initial_balance: f64) -> Result<PaperAccount, sqlx::Error> {
    ...
    pub async fn update_balance(&self, account_id: &str, new_balance: f64) -> Result<(), sqlx::Error> {
        sqlx::query(
            r#"
    ...
    pub async fn reset_account(&self, initial_balance: f64) -> Result<PaperAccount, sqlx::Error> {
        sqlx::query("DELETE FROM paper_positions").execute(&self.pool).await?;
        sqlx::query("DELETE FROM paper_trades").execute(&self.pool).await?;
        sqlx::query("DELETE FROM paper_accounts").execute(&self.pool).await?;
    ...
    pub async fn get_trade_history(&self, account_id: &str) -> Result<Vec<PaperTrade>, sqlx::Error> {
    ...
    pub async fn get_all_positions(&self, account_id: &str) -> Result<Vec<PaperPosition>, sqlx::Error> {
        sqlx::query_as::<_, PaperPosition>(
                ...
                "sell" => {
                    ...
                    while qty_remaining > MINIMUM_QUANTITY {
                        if let Some(front) = entry.front_mut() {
                            ...
                            trade_pnl += (trade.price - front.price) * matched_qty - sell_fee - buy_fee;
...
impl PaperTradingManager {
    ...
    pub async fn execute_trade(
        ...
        db_read
            .update_balance(&account.id, match request.side {
        ...
        match existing {
            Some(mut position) => match request.side {
                OrderSide::Buy => {
                    ...
                    position.unrealized_pnl = (execution_price - position.entry_price) * position.quantity;
    ...
    pub async fn reset_account(&self, initial_balance: Option<f64>) -> Result<PaperAccount, String> {
        let db_read = self.db.read().await;
        db_read
...
mod tests {
    ...
    async fn create_manager_with_configs(
        ...
        PaperTradingManager::with_config(Arc::new(RwLock::new(database)), slippage_config, fee_config)
    }
    ...
    async fn test_virtual_balance_updates() {
        let manager = create_manager_with_configs(deterministic_slippage_config(), FeeConfig::default()).await;
    ...
    async fn test_fee_calculations() {
        ...
        assert!((fees.total_fee - (fees.trading_fee + fees.network_fee + fees.price_impact_fee)).abs() < 1e-9);
    }
