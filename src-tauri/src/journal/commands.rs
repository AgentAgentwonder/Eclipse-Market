...
pub async fn update_journal_entry(
    ...
    let db_lock = db.write().await;
    db_lock.update_entry(&entry).await.map_err(|e| e.to_string())?;
...
pub async fn get_journal_entries(
    ...
    let db_lock = db.read().await;
    db_lock.get_entries(&filters, limit, offset).await.map_err(|e| e.to_string())
}
...
pub async fn get_journal_entries_count(
    ...
    let db_lock = db.read().await;
    db_lock.get_entries_count(&filters).await.map_err(|e| e.to_string())
}
...
pub async fn generate_weekly_report(
    ...
    let db_lock = db.read().await;
    let entries = db_lock.get_entries(&filters, 1000, 0).await.map_err(|e| e.to_string())?;
    drop(db_lock);
    
    let report = JournalAnalytics::generate_weekly_report(&entries);
    
    let db_lock = db.write().await;
    db_lock.save_weekly_report(&report).await.map_err(|e| e.to_string())?;
...
pub async fn get_weekly_report(
    ...
    let db_lock = db.read().await;
    db_lock.get_weekly_report(week_start).await.map_err(|e| e.to_string())
}
...
pub async fn get_weekly_reports(
    ...
    let db_lock = db.read().await;
    db_lock.get_weekly_reports(limit).await.map_err(|e| e.to_string())
}
...
pub async fn get_behavioral_analytics(
    ...
    let db_lock = db.read().await;
    let entries = db_lock.get_entries(&filters, 10000, 0).await.map_err(|e| e.to_string())?;
    drop(db_lock);
...
pub async fn get_journal_stats(
    ...
    let db_lock = db.read().await;
    let total_entries = db_lock.get_entries_count(&all_filters).await.map_err(|e| e.to_string())? as usize;
    let entries_this_week = db_lock.get_entries_count(&week_filters).await.map_err(|e| e.to_string())? as usize;
    let entries_this_month = db_lock.get_entries_count(&month_filters).await.map_err(|e| e.to_string())? as usize;
    
    let all_entries = db_lock.get_entries(&all_filters, 10000, 0).await.map_err(|e| e.to_string())?;
    drop(db_lock);
    
    let total_trades_logged = all_entries.iter()
    ...
    let mut strategy_counts: std::collections::HashMap<String, (usize, usize)> = std::collections::HashMap::new();
    ...
    let most_used_strategies: Vec<StrategyUsage> = {
        let mut strategies: Vec<_> = strategy_counts.into_iter()
            .map(|(tag, (count, wins))| {
    ...
    let most_common_emotions: Vec<EmotionUsage> = {
        ...
        let mut emotions: Vec<_> = emotion_counts.into_iter()
            .map(|(emotion, count)| {
