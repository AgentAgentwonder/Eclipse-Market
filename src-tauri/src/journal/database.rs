...
impl JournalDatabase {
    ...
    pub async fn create_entry(&self, entry: &JournalEntry) -> Result<(), sqlx::Error> {
        ...
        let market_conditions_json = serde_json::to_string(&entry.market_conditions).unwrap_or_default();
        let outcome_json = entry.outcome.as_ref().map(|o| serde_json::to_string(o).unwrap_or_default());
    ...
    pub async fn get_entries(&self, filters: &JournalFilters, limit: i64, offset: i64) -> Result<Vec<JournalEntry>, sqlx::Error> {
        ...
        if let Some(entry_types) = &filters.entry_types {
            if !entry_types.is_empty() {
                let placeholders: Vec<String> = entry_types.iter().map(|_| "?".to_string()).collect();
    ...
    pub async fn save_weekly_report(&self, report: &WeeklyReport) -> Result<(), sqlx::Error> {
        let emotion_breakdown_json = serde_json::to_string(&report.emotion_breakdown).unwrap_or_default();
        let discipline_metrics_json = serde_json::to_string(&report.discipline_metrics).unwrap_or_default();
        let pattern_insights_json = serde_json::to_string(&report.pattern_insights).unwrap_or_default();
        let strategy_performance_json = serde_json::to_string(&report.strategy_performance).unwrap_or_default();
        let psychological_insights_json = serde_json::to_string(&report.psychological_insights).unwrap_or_default();
    ...
    pub async fn get_weekly_report(&self, week_start: i64) -> Result<Option<WeeklyReport>, sqlx::Error> {
        let row = sqlx::query(
    ...
    fn row_to_entry(&self, row: &sqlx::sqlite::SqliteRow) -> JournalEntry {
        JournalEntry {
            ...
            entry_type: serde_json::from_str(row.get("entry_type")).unwrap_or(EntryType::Reflection),
            strategy_tags: serde_json::from_str(row.get("strategy_tags")).unwrap_or_default(),
            emotions: serde_json::from_str(row.get("emotions")).unwrap_or_else(|_| EmotionTracking {
            ...
            notes: row.get("notes"),
            market_conditions: serde_json::from_str(row.get("market_conditions")).unwrap_or_else(|_| MarketConditions {
    ...
    fn row_to_weekly_report(&self, row: &sqlx::sqlite::SqliteRow) -> WeeklyReport {
        WeeklyReport {
            ...
            emotion_breakdown: serde_json::from_str(row.get("emotion_breakdown")).unwrap_or_else(|_| EmotionBreakdown {
            ...
            discipline_metrics: serde_json::from_str(row.get("discipline_metrics")).unwrap_or_else(|_| DisciplineMetrics {
            ...
            strategy_performance: serde_json::from_str(row.get("strategy_performance")).unwrap_or_default(),
            psychological_insights: serde_json::from_str(row.get("psychological_insights")).unwrap_or_else(|_| PsychologicalInsights {
