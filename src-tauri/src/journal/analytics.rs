...
impl JournalAnalytics {
    pub fn generate_weekly_report(entries: &[JournalEntry]) -> WeeklyReport {
        ...
        let average_confidence = if count > 0.0 { total_confidence / count } else { 0.0 };
        
        let emotion_breakdown = EmotionBreakdown {
            emotion_counts,
            average_stress: if count > 0.0 { total_stress / count } else { 0.0 },
            average_clarity: if count > 0.0 { total_clarity / count } else { 0.0 },
            average_fomo: if count > 0.0 { total_fomo / count } else { 0.0 },
            revenge_trading_instances,
        };
        
        let discipline_metrics = DisciplineMetrics {
            average_discipline_score: if count > 0.0 { total_discipline / count } else { 0.0 },
    ...
    fn generate_pattern_insights(entries: &[JournalEntry]) -> Vec<PatternInsight> {
        ...
        let high_stress_losses = entries.iter()
            .filter(|e| {
        ...
        let revenge_trades = entries.iter()
    ...
    fn generate_strategy_performance(strategy_stats: HashMap<String, StrategyStats>) -> Vec<StrategyPerformance> {
        strategy_stats.into_iter()
            .map(|(tag, stats)| {
                ...
                let common_emotions: Vec<String> = stats.emotions.iter()
                    .take(3)
                    .map(|(e, _)| e.clone())
                    .collect();
    ...
    fn generate_psychological_insights(entries: &[JournalEntry]) -> PsychologicalInsights {
        ...
        let dominant_emotions: Vec<String> = {
            ...
            emotions.into_iter().take(3).map(|(e, _)| e.clone()).collect()
        };
        ...
        let (best_mental_state, worst_mental_state) = Self::find_best_worst_mental_states(&best_emotion_wins, &best_emotion_total);
    ...
    fn find_best_worst_mental_states(
        ...
        let mut rates: Vec<_> = totals.iter()
        ...
        let best = rates.first().map(|(e, _)| e.clone()).unwrap_or_else(|| "Calm".to_string());
        let worst = rates.last().map(|(e, _)| e.clone()).unwrap_or_else(|| "Stressed".to_string());
    ...
    fn detect_cognitive_biases(entries: &[JournalEntry]) -> Vec<String> {
        ...
        let recent_losses: Vec<_> = entries.iter()
            .filter(|e| e.outcome.as_ref().map(|o| !o.success).unwrap_or(false))
            .take(3)
            .collect();
        
        if recent_losses.len() >= 2 {
            let revenge_count = recent_losses.iter()
                .filter(|e| e.emotions.revenge_trading)
                .count();
            
            if revenge_count > 0 {
                biases.push("Loss Aversion: Attempting to recover losses quickly through revenge trading".to_string());
            }
        }
        
        let overconfident_losses = entries.iter()
            .filter(|e| {
                e.confidence_level > 0.8 &&
                e.outcome.as_ref().map(|o| !o.success).unwrap_or(false)
            })
            .count();
        
        if overconfident_losses > 2 {
            biases.push("Overconfidence Bias: High confidence trades resulting in losses".to_string());
        }
    ...
    fn generate_recommendations(
        ...
        if patterns.iter().any(|p| p.pattern_type == "Revenge Trading") {
            recommendations.push("Implement a mandatory cool-down period after losses to avoid revenge trading.".to_string());
        }
        
        if recommendations.is_empty() {
            recommendations.push("Great job! Keep maintaining your discipline and emotional control.".to_string());
        }
    ...
    fn calculate_consistency_score(entries: &[JournalEntry]) -> f32 {
        ...
        let days_with_entries = entries.iter()
    ...
    fn calculate_emotional_volatility(entries: &[JournalEntry]) -> f32 {
        ...
        let stress_values: Vec<f32> = entries.iter()
            .map(|e| e.emotions.stress_level)
            .collect();
        
        let mean = stress_values.iter().sum::<f32>() / stress_values.len() as f32;
        let variance = stress_values.iter()
            .map(|v| (v - mean).powi(2))
            .sum::<f32>() / stress_values.len() as f32;
    ...
    fn calculate_best_trading_hours(entries: &[JournalEntry]) -> Vec<usize> {
        ...
        let mut hour_rates: Vec<_> = hour_stats.into_iter()
        ...
        hour_rates.into_iter().take(3).map(|(hour, _)| hour).collect()
    }
    ...
    fn analyze_cognitive_biases(entries: &[JournalEntry]) -> Vec<CognitiveBias> {
        ...
        if fomo_trades > 1 {
            biases.push(CognitiveBias {
                ...
                mitigation_strategy: "Create strict entry criteria and wait for your setups rather than chasing".to_string(),
            });
        }
        ...
        if revenge_trades > 0 {
            biases.push(CognitiveBias {
                ...
                description: "Attempting to recover losses through impulsive revenge trading".to_string(),
                mitigation_strategy: "Implement mandatory breaks after losses and review journal before next trade".to_string(),
            });
        }
    ...
    fn calculate_growth_indicators(entries: &[JournalEntry]) -> GrowthIndicators {
        ...
        let first_discipline = first_half.iter()
            .map(|e| e.emotions.discipline_score)
            .sum::<f32>() / first_half.len() as f32;
        
        let second_discipline = second_half.iter()
            .map(|e| e.emotions.discipline_score)
            .sum::<f32>() / second_half.len() as f32;
        
        let first_stress = first_half.iter()
            .map(|e| e.emotions.stress_level)
            .sum::<f32>() / first_half.len() as f32;
        
        let second_stress = second_half.iter()
            .map(|e| e.emotions.stress_level)
            .sum::<f32>() / second_half.len() as f32;
