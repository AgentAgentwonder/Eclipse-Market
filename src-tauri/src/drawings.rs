...
impl DrawingManager {
    ...
    pub fn list_drawings(&self, symbol: &str) -> Result<Vec<DrawingObject>, String> {
        let all_drawings: Vec<DrawingObject> = self.read_json(&self.drawings_path).unwrap_or_default();
        Ok(all_drawings
            .into_iter()
    ...
    pub fn save_drawings(&self, symbol: &str, drawings: &[DrawingObject]) -> Result<(), String> {
        ...
        let mut all_drawings: Vec<DrawingObject> = self.read_json(&self.drawings_path).unwrap_or_default();
