...
impl TokenFeatures {
    pub fn feature_vector(&self) -> HashMap<String, f64> {
        ...
        features.insert("developer_reputation".to_string(), self.developer_reputation.clamp(0.0, 1.0));
        features.insert(
            "developer_launch_experience".to_string(),
            (self.developer_launch_count as f64 + 1.0).ln().clamp(0.0, 5.0),
        );
        features.insert("developer_success_rate".to_string(), self.developer_success_rate.clamp(0.0, 1.0));
        features.insert("contract_complexity".to_string(), self.contract_complexity.clamp(0.0, 1.0));
        features.insert(
            "proxy_pattern_detected".to_string(),
            if self.proxy_pattern_detected { 1.0 } else { 0.0 },
        );
        features.insert(
        ...
        features.insert("liquidity_ratio".to_string(), self.liquidity_ratio.clamp(0.0, 1.5));
        features.insert(
        ...
        features.insert("marketing_hype".to_string(), self.marketing_hype.clamp(0.0, 1.0));
        features.insert(
            "marketing_spend".to_string(),
            (self.marketing_spend_usd + 1.0).ln().clamp(0.0, 15.0),
        );
        features.insert("social_followers_growth".to_string(), self.social_followers_growth.clamp(-1.0, 1.0));
        features.insert("community_engagement".to_string(), self.community_engagement.clamp(0.0, 1.0));
        features.insert("influencer_sentiment".to_string(), self.influencer_sentiment.clamp(-1.0, 1.0));
        features.insert(
            "security_audit".to_string(),
            self.security_audit_score.unwrap_or(0.5).clamp(0.0, 1.0),
        );
        features.insert("dex_depth".to_string(), self.dex_depth_score.clamp(0.0, 1.0));
        features.insert("watchlist_interest".to_string(), self.watchlist_interest.clamp(0.0, 1.0));
        features.insert("retention".to_string(), self.retention_score.clamp(0.0, 1.0));
        features.insert(
    ...
    pub fn early_warnings(&self) -> Vec<EarlyWarning> {
        ...
        if self.liquidity_usd < 75_000.0 {
            warnings.push(EarlyWarning {
                ...
                severity: if self.liquidity_usd < 25_000.0 { "Critical" } else { "High" }.to_string(),
                message: "Liquidity backing is low which increases slippage and exit risk".to_string(),
                detected_at: now.clone(),
            });
        }
        ...
        if self.proxy_pattern_detected {
            warnings.push(EarlyWarning {
                ...
                message: "Upgradeable proxy patterns detected – monitor for governance changes".to_string(),
                detected_at: now.clone(),
            });
        }
        ...
        if self.marketing_hype > 0.75 && self.community_engagement < 0.45 {
            warnings.push(EarlyWarning {
                ...
                severity: "Medium".to_string(),
                message: "Marketing hype exceeds organic engagement suggesting manufactured interest".to_string(),
                detected_at: now.clone(),
            });
        }
        ...
        if self.social_followers_growth > 0.85 && self.watchlist_interest < 0.3 {
            warnings.push(EarlyWarning {
                ...
                severity: "Medium".to_string(),
                message: "Follower spikes are not translating into watchlist conversions".to_string(),
                detected_at: now.clone(),
            });
        }
