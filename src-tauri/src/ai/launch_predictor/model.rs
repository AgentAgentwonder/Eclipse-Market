    ...
    pub fn predict(&self, features: &TokenFeatures) -> LaunchPrediction {
        ...
        for (name, value) in &feature_map {
            if let Some(&weight) = self.weights.get(name) {
                ...
                contributing_features.push((name.clone(), *value, *importance, contribution, impact));
            }
        }
        ...
        contributing_features.sort_by(|a, b| b.2.partial_cmp(&a.2).unwrap_or(std::cmp::Ordering::Equal));

        let feature_scores: Vec<FeatureScore> = contributing_features
            .iter()
            .map(|(name, value, importance, _contribution, impact)| FeatureScore {
...
fn describe_feature(name: &str, value: f64) -> String {
    match name {
        ...
        "developer_launch_experience" => format!("Developer launch experience: {:.1} launches", value.exp()),
