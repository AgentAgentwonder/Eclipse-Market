...
impl CompressionManager {
    ...
    pub async fn decompress_data(&self, record_id: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        // Check cache first
        {
        ...
        // Fetch from database
        let record = sqlx::query_as::<_, CompressedRecord>(
            "SELECT * FROM compressed_data WHERE id = ?1"
        )
        .bind(record_id)
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn get_compression_stats(&self) -> Result<CompressionStats, Box<dyn std::error::Error>> {
