...
impl HistoricalStorage {
    ...
    pub async fn get_price_data(
        ...
        let data = rows
            .into_iter()
            .map(|(timestamp, open, high, low, close, volume)| HistoricalDataPoint {
                timestamp,
                open,
                high,
                low,
    ...
    pub async fn get_cache_stats(&self, symbol: &str) -> Result<HashMap<String, u64>, sqlx::Error> {
        ...
        let count: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM historical_prices WHERE symbol = ?1",
        )
        .bind(symbol)
        .fetch_one(&self.pool)
        .await?;
        ...
        let count: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM historical_orderbooks WHERE symbol = ?1",
        )
        .bind(symbol)
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn clear_old_data(&self, days: i64) -> Result<u64, sqlx::Error> {
        ...
        let result = sqlx::query(
            "DELETE FROM historical_prices WHERE timestamp < ?1",
        )
        .bind(cutoff_time)
        .execute(&self.pool)
        .await?;
        ...
        sqlx::query(
            "DELETE FROM historical_orderbooks WHERE timestamp < ?1",
        )
        .bind(cutoff_time)
        .execute(&self.pool)
        .await?;
