...
impl BackupService {
    ...
    fn encrypt_data(&self, keystore: &Keystore, data: &[u8]) -> Result<EncryptedBackup, BackupError> {
    ...
    fn decrypt_data(&self, keystore: &Keystore, backup: &EncryptedBackup) -> Result<Vec<u8>, BackupError> {
        let key = self.get_or_create_backup_key(keystore)?;
        let cipher = Aes256Gcm::new(GenericArray::from_slice(&key));
    ...
    pub fn delete_provider_config(&self, id: &str) -> Result<(), BackupError> {
        ...
        if configs.len() == original_len {
            return Err(BackupError::CloudProvider(CloudProviderError::NotConfigured));
        }
        self.save_provider_configs_internal(&*keystore, &configs)?;
        Ok(())
    }
